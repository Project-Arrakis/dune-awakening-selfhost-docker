...
for script in sietches.sh extract-partition-catalog.sh despawn-server.sh spawn-server.sh; do
...
activate_line="$(grep -n 'sietches.sh set-active DeepDesert_1 3' "$calls" | cut -d: -f1)"
if [ -z "$profile_line" ] || [ -z "$director_line" ] || [ -z "$activate_line" ] \
  || [ "$profile_line" -ge "$director_line" ] || [ "$director_line" -ge "$activate_line" ]; then
  echo "FAIL: the third PvP selector and Director config were not applied before activation" >&2
  cat "$calls" >&2
  exit 1
fi
...
echo "PASS: Deep Desert layout applies roles before spawning, leaves Hagga online, and blocks connected-player disruption"
