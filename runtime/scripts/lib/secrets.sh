...
#     process kill mid-write cannot produce a torn state for the
#     highest-blast-radius secret this protects (the Postgres
#     superuser password).
...
#   a local connection (host 127.0.0.1, matching this image's own
...
#   psql not found, etc).
#
#   Exists specifically because Postgres's own docker-entrypoint.sh
#   only applies POSTGRES_PASSWORD_FILE during first-time initdb. On an
#   existing data directory -- the real upgrade path, an operator
#   turning the age secrets backend on for a host that already has a
#   live cluster -- the file's value is silently ignored by Postgres,
#   and the encrypted secrets store can end up holding a password that
#   does NOT match what Postgres will actually accept, with no error
#   surfaced at the point the bad value is written. Calling this
#   function after every container start (not just "first time") when
#   the age backend is configured keeps the live database and the
#   encrypted store from ever silently diverging.
#
...
#
#   CRITICAL: this function MUST pipe the SQL statement over STDIN, as
#   below -- never pass the password via `psql -c "ALTER USER ...
#   PASSWORD '...'"`. That form embeds the plaintext password as a
#   literal argv element of the psql process for its entire lifetime,
#   visible via /proc/<pid>/cmdline (both the host-side `docker exec`
#   process and the actual `psql` process running inside the
#   container) to any other local process for as long as it runs --
#   the exact same exposure class (GHSA-fc89-h24v-6j3x) this whole
#   feature exists to eliminate from `docker inspect`. This was found
#   and fixed during development by independently reproducing the
#   exposure live via /proc inspection, exactly the same technique
#   used to find and fix the original docker-inspect exposure this
#   feature closes. The stdin-based form below matches the same
#   discipline already used by
#   _dune_secrets_aead_encrypt/_dune_secrets_aead_decrypt above, and by
#   start-postgres.sh's own pre-existing schema-normalization SQL
...
#   image's pg_hba.conf trusts local connections unconditionally, so an
#   auth check made that way will falsely "pass" even with a wrong
#   password. Verify the password actually took effect via a real
#   network client instead (e.g. `docker run --rm --network dune-net
#   postgres:<tag> psql -h <container> -U <db-user> ...`).
