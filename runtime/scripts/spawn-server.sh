...
# Funcom token is always operator-provided (resolve_funcom_token has no
# auto-generation fallback), so it's the one secret here that can
# genuinely be "missing" in a way that should fail loudly with a clear
# message rather than the generic non-zero exit resolve_funcom_token
# itself would produce. RMQ/FLS secrets are auto-generated by
# resolve_rmq_http_token_auth_secret/resolve_fls_apikey if missing, so
# they cannot fail this way -- checked only via their own resolver's
# success below, not a separate existence check.
if ! FUNCOM_TOKEN="$(resolve_funcom_token)"; then
  echo "Missing required secret: funcom-token (checked runtime/secrets/funcom-token.txt and the encrypted secrets store)"
  exit 1
fi
