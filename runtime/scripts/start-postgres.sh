...
# GHSA-fc89-h24v-6j3x follow-on: this file is generated fresh on every
# run and previously had no explicit permission set, defaulting to the
# umask's usual 644 -- world-readable, containing the dune role's
# password in plaintext SQL. Tighten to 600 immediately after writing,
# regardless of umask.
...
# Docker socket access. Resolved via the shared secrets seam below: if
# age-based secrets are configured (dune_secrets_backend_configured),
...
# single shared render-file path below -- confirmed via direct
# reproduction of exactly this race (instance A's docker run binding
# instance B's password, or A's cleanup deleting the file out from
# under B) with no lock in place, before this lock was added.
...
(
  ...
  if dune_secrets_backend_configured; then
    ...
    # a real one.
    #
    # A bare `2>/dev/null` here would suppress EVERY diagnostic from
  ...
  #
  # CRITICAL, corrected after a real, live-reproduced production
  # incident: this file is intentionally NOT deleted once the
  # container starts,
  ...
  # because it defends against a second real, reproduced production
  # incident: a stray non-file (e.g. a directory Docker itself left
  # behind at this exact path under the earlier failure condition
  # described above) makes a naive `rm -f` exit non-zero, which aborts
  # this script under `set -euo pipefail` BEFORE `docker run` ever
  # executes -- confirmed live, this left dune-postgres down. See
  # secrets.sh's own docstring for the self-healing behavior and why
  # it lives there, not duplicated inline in every script that renders
  # a short-lived bind-mount file like this one.
  ...
  ready=0
  # shellcheck disable=SC2034  # loop counter only used to bound retries, not read
  ...
  # POSTGRES_PASSWORD_FILE is only applied by Postgres's own
  # docker-entrypoint.sh during first-time initdb. On an existing data
  # directory -- the real upgrade path: an operator turning the age
  # backend on for a host that already has a live cluster -- the
  ...
  # data directory, including hitting a real pg_hba.conf
  ...
  # password is currently set. This runs on every startup when the age
  # backend is configured, not only "first time" -- so the live
  # password and the encrypted store can never silently diverge,
  # whether this is a fresh install, an upgrade of an existing install,
  # or a manual secret rotation.
  #
  ...
  # Fail loudly instead of continuing.
  if dune_secrets_backend_configured; then
