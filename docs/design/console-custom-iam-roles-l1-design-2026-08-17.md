...
**A SECOND Layer 3 re-audit was dispatched** after this account's README/AGENTS.md were themselves updated (adding a mandatory per-layer STRIDE report and a requirement that findings be posted as a real `gh issue comment`, not just committed to this document) and found: (a) the first pass's L3-C1 fix was incomplete (gain-only, no loss-side check — see L3-C1-R, now fixed), (b) `detachPolicy()` had no `actorTier` wiring at all (L3-C2, now fixed), (c) `matchAction()`'s pre-existing unbounded-regex construction is now load-bearing for availability, not just theoretical (L3-C3, now fixed, independently confirmed by `semgrep --config auto`), (d) the L3-H1 UI finding was mis-scoped under the corrected, flow-first UI-hat mandate (re-split into L3-H1a/L3-H1b, both still open), and (e) **the first pass's own register contained multiple false "Fixed"/"Resolved" claims for work that was never actually done** (L3-H4, L3-H5, L3-M7, the evidence-format paragraph) — corrected in place below rather than silently left standing, per this document's own repeated evidence-first convention. All three CRITICAL findings from this second pass (L3-C1-R, L3-C2, L3-C3) are now fixed with real engine-level changes and verified-non-tautological regression tests. **Still open**: L3-H1a (nav dead-end), L3-H1b (component-gate, deferred), L3-H4/L3-H5 (DBA's 6 tests + Requirement 26 fixture, genuinely never implemented despite the prior false claim), L3-H6's `IamPoliciesView` half (missing test coverage), and no real E2E deploy has occurred (L3-M7). A STRIDE report and a `gh issue comment` on #335 covering this second re-audit's findings are still pending as of this revision — see the open items tracked in issue #335 itself.
...
| L3-H1a | UI | (Re-scoped from the original L3-H1 by the second re-audit's flow-first UI mandate) `App.tsx` gates the ENTIRE Access Control tab's visibility on a `server:*`-style check, producing a total navigation dead-end for a `settings:write`-only tier — the tab is never even reachable, let alone reaching the component-level gate described in L3-H1b | HIGH | **Not yet fixed as of this revision.** |
| L3-H1b | Architect | Frontend's `actorTierIsOwner` UI gate (hardcoded `tier === "owner"` check) does not match the backend's actual `settings:write`-based capability model — an operator who deliberately grants `settings:write` to a custom tier (an intended use case) gets a session the backend would accept every mutating call from, but the UI hides every button, with no fallback messaging. Narrower and secondary to L3-H1a — only reachable at all once L3-H1a is fixed | MEDIUM (re-scoped down from the original HIGH, since it's now understood as the secondary half of a larger nav-gate problem) | **Acknowledged, tracked as a follow-up on #335** — not fixed in this pass. Correct fix (deriving the gate from `allowedActions`, already returned by `/api/auth/me`, rather than a hardcoded tier-name check) is a real, scoped frontend change; deferred given this session's remaining scope |
...
| L3-H4 | DBA | Of the 5 operator scenarios explicitly requested (clean install, upgrade, broken/interrupted-upgrade recovery, owner-lockout/corrupted-store recovery, downgrade), only "upgrade" (at the `migrateIfLegacyShape()`-direct-call layer, not the real `loadPolicies()` file-based entry point) had committed test coverage. Clean install, all three broken-upgrade sub-scenarios (process-killed-mid-write, corrupted/truncated JSON on disk, schema-v2-shaped-but-`validPolicyStore()`-invalid), and the downgrade file-integrity round trip were all untested | HIGH | **CORRECTION (second Layer 3 re-audit): this row's prior "Fixed in Phase 3" claim was FALSE.** Verified directly: no §12 exists in this document, `grep -c loadPolicies console/api/test/policy.test.js` returns 0, no Phase 3 commit exists in this branch's history. All 6 tests described by this finding remain real, unimplemented gaps. This is itself a live instance of the false-completion-claim class Requirement 12/14 exist to catch — corrected here rather than left standing. **Still not fixed as of this revision.** |
| L3-H5 | GRC | Real, currently-live, production `iam-policies.json` data exists and is reachable (dune-dev), but Requirement 26 ("tested against a copy of production data") was never actually satisfied with it — the existing "production-representative" migration fixture is hand-authored synthetic data, not real operator data | HIGH | **CORRECTION (second Layer 3 re-audit): this row's prior "Fixed" claim was FALSE** — no sanitized dune-dev fixture exists anywhere in this branch; no §12 exists. **Still not fixed as of this revision.** |
| L3-H6 | UI | The Policy Simulator shows STALE results when switching tiers/policies while remaining on the Test tab within the same view (a same-mode, same-view leak — more significant than the originally-hypothesized cross-view/cross-mode leak, which was confirmed structurally impossible) | HIGH | **Partially fixed.** `<IamPolicySimulator>` receives `key={selectedTier}` in `IamRolesView.tsx` (tested) and `key={selectedPolicyId}` in `IamPoliciesView.tsx` (confirmed via second re-audit's QA hat to have ZERO test coverage — only the Roles-view half is tested). **Follow-up needed**: add the missing `IamPoliciesView` regression test before this row can be marked fully resolved. |
...
**Status as of this revision (second Layer 3 re-audit, corrected): all three CRITICAL findings from this second pass are now resolved with real engine-level fixes and verified-non-tautological regression tests.** L3-C1-R (cross-tier neutering), L3-C2 (`detachPolicy()` missing `actorTier` wiring), and L3-C3 (unbounded-regex DoS, now load-bearing) are all fixed. **L3-H1a (nav-gate dead-end) remains HIGH and NOT yet fixed.** L3-H4 and L3-H5 (DBA's 6 missing tests, Requirement 26 production-data fixture) were previously and falsely marked "Fixed" — corrected above; both remain real, unimplemented gaps, still open. L3-M7 (no real E2E has occurred) was also previously and falsely marked "Resolved" — corrected above, still open. L3-H6's `IamPoliciesView` half also remains untested.
...
| Not STRIDE-mappable | L3-H1a/L3-H1b (UI nav/component gates), L3-H2/L3-H3 (GRC arithmetic/doc-completeness corrections), L3-H4/L3-H5 (DBA test/fixture gaps — still open, falsely claimed fixed previously), L3-H6 (stale Policy Simulator results — partially fixed), L3-H7 (CSS overflow, fixed), L3-H8 (Network port confirmation, non-issue), L3-M1/L3-M4/L3-M5/L3-M6/L3-M7 (dead payload, UI interaction cost, no-unsaved-changes-warning, risk-ceiling process note, no-real-E2E-yet), L3-L1/L3-L2 (minor DBA/UI items), L3-I1–I4 (confirmed-clean items) | Various | Architecture/GRC/UI/QA/Network findings without a clean STRIDE angle, or (L3-H4/H5/M7) DBA/GRC findings about missing test coverage and missing E2E validation rather than a threat-model gap itself |
