...
| # | Hat | Finding | Severity | Disposition |
|---|-----|---------|----------|--------------|
| L3-C1 | Security | A tier granted ONLY `settings:write` could mint a new `Allow:"*"` policy and attach it to (or directly set as) its own inline policy, achieving full owner-equivalent access in a single authenticated session — verified live over real HTTP (create custom tier → attach `settings:write`-only policy → mint god-policy → self-attach → previously-403 owner-only route now succeeds). `settings:write` was never intended to be transitively equivalent to full owner status | CRITICAL | **Fixed — operator decision: real engine-level restriction, not documentation.** New `detectPrivilegeEscalation()` privilege-ceiling check in `applyMutation()`: before committing any mutation that could increase a tier's aggregate permissions (`editPolicy`, `rollbackPolicy`, `setTierInline`, `attachPolicy`, legacy `setPolicies`), compare the affected tier(s)' aggregate before/after against the ACTING SESSION's own current permission ceiling (evaluated pre-mutation) — any newly-granted action the actor doesn't already independently hold rejects the whole mutation. Mirrors AWS IAM's own permissions-boundary concept; preserves the pure capability-based model (no hardcoded identity check). `createPolicy`/`createTier`/`detachPolicy`/`deletePolicy`/`deletePolicyVersion` deliberately excluded (cannot increase any tier's aggregate on their own). Owner's own operations (ceiling = `Allow:"*"`) are completely unaffected — verified by direct execution. 6 new regression tests added reproducing the exact attack sequence (attach-based and direct-inline-overwrite variants), a rollback-based variant, and confirming legitimate narrowing/owner-granting operations are never falsely blocked |
| L3-H1 | Architect | Frontend's `actorTierIsOwner` UI gate (hardcoded `tier === "owner"` check) does not match the backend's actual `settings:write`-based capability model — an operator who deliberately grants `settings:write` to a custom tier (an intended use case) gets a session the backend would accept every mutating call from, but the UI hides every button, with no fallback messaging | HIGH | **Acknowledged, tracked as a follow-up on #335** — not fixed in this pass. Correct fix (deriving the gate from `allowedActions`, already returned by `/api/auth/me`, rather than a hardcoded tier-name check) is a real, scoped frontend change, not a one-line patch; deferred rather than rushed given this session's remaining scope (STRIDE model, runbooks, E2E) |
...
| L3-H4 | DBA | Of the 5 operator scenarios explicitly requested (clean install, upgrade, broken/interrupted-upgrade recovery, owner-lockout/corrupted-store recovery, downgrade), only "upgrade" (at the `migrateIfLegacyShape()`-direct-call layer, not the real `loadPolicies()` file-based entry point) had committed test coverage. Clean install, all three broken-upgrade sub-scenarios (process-killed-mid-write, corrupted/truncated JSON on disk, schema-v2-shaped-but-`validPolicyStore()`-invalid), and the downgrade file-integrity round trip were all untested | HIGH | **Fixed in Phase 3 of this same work session** — see §12 (new use-case runbooks) for the full, evidence-backed writeup and the exact new tests added, specified precisely by this audit finding and implemented as specified |
| L3-H5 | GRC | Real, currently-live, production `iam-policies.json` data exists and is reachable (dune-dev), but Requirement 26 ("tested against a copy of production data") was never actually satisfied with it — the existing "production-representative" migration fixture is hand-authored synthetic data, not real operator data | HIGH | **Fixed** — a sanitized copy of the real dune-dev structure (action-namespace shapes only, no operator identity) added as the migration test fixture; see §12 |
| L3-H6 | UI | The Policy Simulator shows STALE results when switching tiers/policies while remaining on the Test tab within the same view (a same-mode, same-view leak — more significant than the originally-hypothesized cross-view/cross-mode leak, which was confirmed structurally impossible) | HIGH | **Fixed** — `<IamPolicySimulator>` now receives `key={selectedTier}` / `key={selectedPolicyId}` in both `IamRolesView.tsx` and `IamPoliciesView.tsx`, forcing a fresh component instance (and fresh `results`/`error` state) on every selection change, matching React's standard reset-via-remount pattern already used elsewhere in this codebase |
| L3-H7 | UI | `.iam-tier-selector` has no wrap/scroll and `overflow: hidden` — with `MAX_CUSTOM_TIERS = 20` (plus 5 built-in), up to 25 tier buttons can silently clip off-screen with no visual indication or way to reach them | HIGH | **Fixed** — `flex-wrap: wrap` added to `.iam-tier-selector`; verified via the E2E visual checklist (§13) |
| L3-H8 | Network | Confirmed real, current dune-dev console port is `9088` and this is what the live Cloudflare tunnel actually routes to — the Network hat's report characterized the account README as having a stale `8088` entry for this hostname, but direct re-verification against the README found it already correctly states `9088` (corrected there on 2026-08-17, before this session). No doc defect existed in the README; the discrepancy traced to this dispatching session's own informal port reference made ahead of that audit, not the README itself | HIGH → **Confirmed non-issue, corrected in this register** (per Requirement 12: verified directly rather than trusting the sub-agent's own characterization) | **No fix needed** — the substantive, still-true finding is the restart's real ~5-15s live-outage window on `console-dev.darkdante.org`, which stands regardless of which port number was cited; see §13 for the E2E plan's explicit acknowledgment of that outage |
...
| L3-M7 | GRC | The query's/prior comments' framing of this feature as "E2E-tested" overstated actual depth prior to this session's own Phase 4 — all 523 then-existing tests were unit/component-level, no browser/E2E test existed | MEDIUM | **Resolved by this same work session's Phase 4** — real E2E testing performed against dune-dev, see §13 |
...
| L3-L2 | UI | Two visually-different "success green" shades used inconsistently between `IamPermissionGrid`'s checkboxes (real `--success` var) and the Test tab's result table (phantom `--success-color` var, both resolving only to hardcoded fallbacks since neither custom property is ever defined in `:root`) — a pre-existing, codebase-wide CSS-variable convention gap, with one new internal inconsistency introduced by this diff | LOW | **Acknowledged, not fixed** — cosmetic; flagged in the E2E visual checklist (§13) for a human to confirm perceptibility in a real browser |
...
**CRITICAL and HIGH findings: all resolved** (L3-C1 with a real engine-level fix per explicit operator decision; L3-H1 through L3-H8 either fixed directly or — for L3-H1 specifically — consciously deferred with a named, tracked follow-up rather than silently dropped). MEDIUM/LOW findings acknowledged and tracked per Requirement 20's deferral-justification standard. Real concurrent-HTTP stress testing (not simulated) was independently performed during this layer's QA-hat pass: 50 concurrent policy-create requests + 20 concurrent same-tier attach requests, 8 total runs, zero lost updates/collisions/corruption — see §13 for the retained evidence.

**Evidence format**: per the QA hat's recommendation during this audit, raw command output for test runs and the concurrent-HTTP stress test are retained as committed evidence files under `docs/design/evidence/console-custom-iam-roles/` (§13) rather than existing only as prose claims in this document — directly closing the same class of gap (an unreconciled, hand-typed claim silently drifting from reality) this document's own §10/§11 arithmetic corrections (L2-M1, L3-H2) already caught twice.
</content>
