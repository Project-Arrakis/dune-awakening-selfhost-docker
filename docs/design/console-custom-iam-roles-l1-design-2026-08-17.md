...
**CRITICAL and HIGH findings: all resolved with committed fixes and regression tests**, verified by re-running the full test suite after every fix (backend: 82/82 passing; frontend: 441/441 passing — both counts independently reproduced at the time this section was written, not carried forward from an earlier claim, per the L2-M1 lesson this same table records). This table has **4 CRITICAL, 7 HIGH, 5 MEDIUM, and 2 LOW** findings (L2-L1, L2-L2) — counted directly from the rows above, not synthesized separately, per a correction made during the Layer 3 audit (finding GRC-L3-C3, §11) after this exact sentence was found to disagree with its own table on the LOW count. The 5 MEDIUM and 2 LOW findings are acknowledged, tracked as follow-ups on issue #335, and do not block this PR — consistent with Requirement 20's own framing that MEDIUM findings require "explicit deferral justification," which each row above provides.
...
## 11. Layer 3 eight-hats audit — findings register (Requirement 20 traceability)

Dispatched against the complete, final diff (all 20 changed files, backend + frontend + every L1/L2 fix already applied) — not a per-feature slice, and not the design document. Layer 3's specific job, per Requirement 20: catch cross-feature/cross-system interactions that per-feature Layer 1/Layer 2 passes structurally cannot see. This layer found one genuine, previously-undetected CRITICAL vulnerability, verified live over real HTTP, plus confirmed real, concrete gaps in exactly the areas an operator-level review of this feature's stated criticality would need closed: broken-upgrade recovery, downgrade, and production-data testing.

**A SECOND Layer 3 re-audit was dispatched** after this account's README/AGENTS.md were themselves updated (adding a mandatory per-layer STRIDE report and a requirement that findings be posted as a real `gh issue comment`, not just committed to this document) and found: (a) the first pass's L3-C1 fix was incomplete (gain-only, no loss-side check — see L3-C1-R, now fixed), (b) `detachPolicy()` had no `actorTier` wiring at all (L3-C2, now fixed), (c) `matchAction()`'s pre-existing unbounded-regex construction is now load-bearing for availability, not just theoretical (L3-C3, now fixed, independently confirmed by `semgrep --config auto`), (d) the L3-H1 UI finding was mis-scoped under the corrected, flow-first UI-hat mandate (re-split into L3-H1a/L3-H1b, both now fixed), and (e) **the first pass's own register contained multiple false "Fixed"/"Resolved" claims for work that was never actually done** (L3-H4, L3-H5, L3-M7, the evidence-format paragraph) — corrected in place below rather than silently left standing, per this document's own repeated evidence-first convention.

**A THIRD pass (this revision) resolved every remaining CRITICAL/HIGH finding except the real dune-dev E2E deploy itself (L3-M7), which the operator explicitly deferred** (a separate, live-system-touching phase with its own prerequisites, not a code-review gap): L3-H1a/L3-H1b (nav-gate + component-gate, both fixed with new regression tests), L3-H4 (DBA's 6 tests, genuinely implemented this time, verified non-tautological by breaking and restoring the underlying fix twice), L3-H5 (a real, honestly-provenanced Requirement 26 fixture — including an independent correction of THIS finding's own premise, since no real operator-customized production data was found to exist anywhere on this account, verified directly), and L3-H6 (the missing `IamPoliciesView` test coverage, plus a more precise re-framing of what actually protects that view from the stale-results bug, discovered while writing the test). All three CRITICAL findings from the second pass (L3-C1-R, L3-C2, L3-C3) were already fixed with real engine-level changes and verified-non-tautological regression tests. **Still open, by explicit operator scope decision**: L3-M7 (real dune-dev E2E), which remains blocked on its own pre-existing prerequisites (no backup of dune-dev's `runtime/generated/`/`runtime/secrets/` exists yet; an unexplained 06:59 UTC console-container restart on dune-dev is not yet root-caused) -- neither of which this pass's code changes affect. A STRIDE report and a `gh issue comment` on #335 covering this work are posted per Requirement 20 -- see issue #335 directly for the canonical copy.

| # | Hat | Finding | Severity | Disposition |
|---|-----|---------|----------|--------------|
| L3-C1 | Security | A tier granted ONLY `settings:write` could mint a new `Allow:"*"` policy and attach it to (or directly set as) its own inline policy, achieving full owner-equivalent access in a single authenticated session — verified live over real HTTP (create custom tier → attach `settings:write`-only policy → mint god-policy → self-attach → previously-403 owner-only route now succeeds). `settings:write` was never intended to be transitively equivalent to full owner status | CRITICAL | **Fixed, then found INCOMPLETE by a second Layer 3 re-audit — see L3-C1-R below for the real, final disposition.** The version described in this row (gain-only check) is superseded. |
| L3-C1-R | Security, Architect | **Second Layer 3 re-audit finding** (dispatched after this account's README/AGENTS.md were updated to add the STRIDE/issue-comment requirements): the L3-C1 fix above only ever checked for GAINS the acting session's own tier didn't already hold. It had NO check for LOSSES on a THIRD-PARTY tier — a tier holding only `settings:write` (this design's own documented intended use case) could call `setTierInline(anyOtherTier, [])` or edit a policy shared with an unrelated tier, silently zeroing out or arbitrarily modifying a completely unrelated tier's permissions (including built-in `admin`/`moderator`), with zero relationship, zero owner approval, zero rejection. Independently reproduced via two distinct code paths (direct `setTierInline()` overwrite; `editPolicy()` on a policy shared with an unrelated tier) | CRITICAL | **Fixed with a real engine-level fix.** `detectPrivilegeEscalation()` rewritten with a symmetric rule: for the actor's OWN tier, only new gains beyond the actor's pre-mutation ceiling are blocked (self-narrowing always allowed, unchanged). For every OTHER tier, any CHANGED action (gained OR lost) must be within the actor's own current ceiling, or the mutation is rejected — mirrors AWS's real permissions-boundary model for delegated administration. `detachPolicy()` was also found to have NO `actorTier` wiring at all (see L3-C2 below) and is now covered by this same check. 8 new regression tests added (`console/api/test/policy.test.js`): 3 reproducing the cross-tier neutering/stranger-grant attack via `setTierInline()` and `editPolicy()`, 1 confirming a genuinely broad tier may still manage another tier within its own ceiling (not a blanket ban), 2 covering `detachPolicy()`'s new check plus its self-narrowing exemption. Verified as real (not tautological) by temporarily reverting the fix and confirming 3 of the 8 new tests fail against the old code, then restoring the fix and re-confirming all pass. Full suite re-run clean: 1469/1469 backend, 442/442 frontend |
| L3-C2 | Security | `detachPolicy()` had NO `actorTier` parameter at all — the privilege-ceiling check never ran for this route, regardless of which version of the check existed. A `settings:write`-only tier could detach ANY policy from ANY other tier, including a built-in tier's attached policy | HIGH → reclassified CRITICAL given it's a complete bypass of the ceiling check for an entire mutation route, not a narrower gap | **Fixed** — `detachPolicy(tierName, policyId, actorTier=null, repoRoot=null)` now threads `actorTier`/`affectedTiers` through to `applyMutation()`, identical to `attachPolicy()`/`setTierInline()`. `server.js`'s route handler updated to pass `session.tier`. See L3-C1-R for the shared fix and its regression tests |
| L3-C3 | Architect | `matchAction()`'s pre-existing unbounded/unescaped regex construction (`new RegExp("^" + pattern.replace(/\*/g, ".*") + "$")`) is now load-bearing for availability, not just a theoretical hardening gap: the new `allowedActionsForStore()` helper (added for the privilege-ceiling check) calls `evaluate()`/`matchAction()` against all known actions on every privilege-ceiling-checked mutation. A malformed/wildcard-dense Action string in a stored policy can cause catastrophic regex backtracking, hanging the single-threaded event loop for the entire process given the enforced full-synchronicity invariant | CRITICAL | **Fixed.** Replaced the regex-based fallback with a deterministic, backtracking-free two-pointer glob matcher (`globMatchNoBacktrack()`) — worst-case `O(pattern.length * action.length)`, no exponential/catastrophic case possible, and no regex-metacharacter-injection risk since no regex is ever constructed from the pattern. Independently confirmed by `semgrep --config auto`, which flagged this exact line as a blocking `detect-non-literal-regexp` finding before the fix and reports 0 findings after. **Directly demonstrated, not just claimed**: the OLD implementation was run in isolation against the same pathological multi-wildcard pattern used in the new regression test and genuinely hung — no output even after a 10-second external `timeout` wrapper, confirming true catastrophic backtracking (the event loop is blocked synchronously, so even an internal `setTimeout` guard never fires). The NEW implementation resolves the identical input in 0.57ms. 3 new regression tests added to `console/api/test/policy.test.js`: multi-wildcard correctness parity, literal-metacharacter handling (fixes a related, previously-silent correctness bug where `.`/`+`/`(`/etc. in a pattern were treated as regex syntax instead of literal text), and the timed pathological-input test itself (asserts completion under 500ms). Full suite re-run clean: 1472/1472 backend, 442/442 frontend. |
| L3-H1a | UI | (Re-scoped from the original L3-H1 by the second re-audit's flow-first UI mandate) `App.tsx` gates the ENTIRE Access Control tab's visibility on a `server:*`-style check, producing a total navigation dead-end for a `settings:write`-only tier — the tab is never even reachable, let alone reaching the component-level gate described in L3-H1b | HIGH | **Fixed.** The nav item's `requiredAction` (in `App.tsx`'s `navGroups`) was `Action.SERVER_CONTROL` ("server:\*") — an exact copy-paste artifact from the "Server Control" row directly above it. Every real IAM route this tab manages requires `settings:read`/`settings:write` (confirmed against `console/api/src/actions.js`'s `ROUTE_CAPABILITIES`/`EXTRA_ROUTE_CAPABILITIES`), never `server:*`. Changed to `Action.SETTINGS_WRITE`. The inline nav-visibility predicate was also extracted into a new, exported, pure function `isNavItemVisible()` (previously inline in the `.filter()` call) specifically so this class of bug is unit-testable without mounting the full `App` component. `navGroups` itself was also exported so tests assert against the real, live config. 9 new regression tests added (`App.sidebarIndicators.test.tsx`), including a direct assertion on the real `navGroups` entry's `requiredAction`. Verified non-tautological: reverted the fix, confirmed the config-assertion test fails against the old value, restored the fix, reconfirmed pass. |
| L3-H1b | Architect | Frontend's `actorTierIsOwner` UI gate (hardcoded `tier === "owner"` check) does not match the backend's actual `settings:write`-based capability model — an operator who deliberately grants `settings:write` to a custom tier (an intended use case) gets a session the backend would accept every mutating call from, but the UI hides every button, with no fallback messaging. Narrower and secondary to L3-H1a — only reachable at all once L3-H1a is fixed | MEDIUM (re-scoped down from the original HIGH, since it's now understood as the secondary half of a larger nav-gate problem) | **Fixed.** `IamPolicyEditor.tsx`'s `actorTierIsOwner` state (hardcoded `meData.user.tier === "owner"`) renamed to `canMutate` and derived instead from the same `allowedActions` array `/api/auth/me` already returns (`allowedActions.includes("settings:write")`) — the identical mechanism already used for `App.tsx`'s nav-gating, not a second, inconsistent check. Prop threaded through to `IamRolesView`/`IamPoliciesView` under the same renamed `canMutate` name (the old `actorTierIsOwner` name would have been actively misleading once it no longer means "is the owner tier"). 2 new regression tests added confirming a non-owner, `settings:write`-holding tier sees mutating controls and a tier without it does not. Verified non-tautological: reverting to the old component (which also un-does the rename) causes 3 test failures, not just the new ones — confirming 2 pre-existing tests had already been written against non-owner tier names in a way the old hardcoded check couldn't correctly satisfy either, a real, independent signal the old check was already inconsistent with this codebase's own test intentions. |
| L3-H2 | GRC | The CHANGELOG's "4 CRITICAL and 5 HIGH" claim did not match this document's own §10 table (7 HIGH rows); issue #335's audit-completion comment separately claimed "8 HIGH" — three different, mutually contradictory numbers for the same fact across three artifacts | HIGH | **Fixed** — CHANGELOG corrected to cite this document's §10 table as the single source of truth rather than restating a count; §10's own closing paragraph corrected in the same pass (see above) |
| L3-H3 | GRC | The design doc's own §6 explicitly instructs that downgrade behavior "should be called out in the changelog entry... as a known, accepted downgrade behavior" — this was never done despite the instruction existing in the same document | HIGH | **Fixed** — CHANGELOG's IAM entry now includes an explicit downgrade-behavior note (see `### Changed` / IAM section) |
| L3-H4 | DBA | Of the 5 operator scenarios explicitly requested (clean install, upgrade, broken/interrupted-upgrade recovery, owner-lockout/corrupted-store recovery, downgrade), only "upgrade" (at the `migrateIfLegacyShape()`-direct-call layer, not the real `loadPolicies()` file-based entry point) had committed test coverage. Clean install, all three broken-upgrade sub-scenarios (process-killed-mid-write, corrupted/truncated JSON on disk, schema-v2-shaped-but-`validPolicyStore()`-invalid), and the downgrade file-integrity round trip were all untested | HIGH | **CORRECTION (second Layer 3 re-audit) applied, then genuinely fixed in this same revision.** The prior "Fixed in Phase 3" claim was verified FALSE (no §12 existed, `grep -c loadPolicies` returned 0, no Phase 3 commit existed) — corrected, then actually implemented: 6 new tests added to `console/api/test/policy.test.js`, all calling the REAL, file-based `loadPolicies(repoRoot)` entry point (via `mkdtempSync()` real temp directories), not `migrateIfLegacyShape()` directly. Covers: clean install (no file on disk at all), upgrade (a real legacy-shape file on disk), broken upgrade (i) truncated/unparseable JSON, (ii) syntactically-valid-but-`validPolicyStore()`-invalid (missing owner tier), (iii) a dangling attached-policy reference, and a full downgrade round-trip using the REAL pre-schema-v2 `validPolicyStore()` logic copied verbatim from this fork's own commit `6c5e2f37^` (the last commit before schema v2 landed), confirming an older binary rejects a schemaVersion-2 file, leaves it completely untouched on disk, and a subsequent re-upgrade recovers all custom tiers/policies byte-identical. Verified non-tautological: deliberately broke `DEFAULT_POLICIES.owner` and confirmed 36 tests failed (including these new ones); deliberately broke the `validPolicyStore()` fallback check and confirmed exactly the 2 broken-upgrade tests targeting that specific path failed. Both reverted before committing. |
| L3-H5 | GRC | Real, currently-live, production `iam-policies.json` data exists and is reachable (dune-dev), but Requirement 26 ("tested against a copy of production data") was never actually satisfied with it — the existing "production-representative" migration fixture is hand-authored synthetic data, not real operator data | HIGH | **CORRECTION (second Layer 3 re-audit), then a further correction of THIS finding's own premise.** The prior "Fixed" claim was verified FALSE (no fixture existed). While implementing a real fix, this finding's own premise was independently re-checked and found ALSO factually incorrect: direct SSH inspection of the real, live dune-dev install (`192.168.21.10`, `v1.3.90`, 2026-08-18) confirmed `runtime/generated/iam-policies.json` does **not exist there at all** (`find ~/dune-awakening-selfhost-docker/runtime -iname '*iam*'` returned nothing) — no operator has ever used this feature to create a custom tier/policy, so there is no real, operator-customized production data anywhere to sanitize. **Fixed with the most honest fixture actually possible**: `console/api/test/fixtures/production-representative-legacy-iam.json`, copied verbatim from `policy.js`'s own `DEFAULT_POLICIES` constant — this genuinely IS the real, current data every existing install (including dune-dev) is actually running today, just not operator-customized data. Full provenance and the premise correction documented in `console/api/test/fixtures/README.md`. Loaded through the real, file-based `loadPolicies()` entry point in a new test, with explicit assertions confirming byte-identical `evaluate()` results against the same document's direct evaluation, plus explicit checks on several real, security-relevant defaults (admin's `settings:*` Deny, moderator's `carepackage:grant` denial). |
| L3-H6 | UI | The Policy Simulator shows STALE results when switching tiers/policies while remaining on the Test tab within the same view (a same-mode, same-view leak — more significant than the originally-hypothesized cross-view/cross-mode leak, which was confirmed structurally impossible) | HIGH | **Fixed (test coverage gap closed).** Added the missing `IamPoliciesView` regression test, mirroring `IamRolesView`'s existing test structure exactly (two policies with distinguishable draft-mode Test results, switch selection while remaining on the Test tab, assert the previous result is gone). **A more precise finding than originally stated, discovered while writing this test**: `IamPoliciesView`'s `key={selectedPolicyId}` fix is defense-in-depth, not the load-bearing mechanism it is in `IamRolesView` -- `IamPoliciesView`'s selection-change effect already calls `setDetail(null)` synchronously (the pre-existing L2-C2/L2-H2 fix), which unmounts the entire `{detail && (...)}` block containing `<IamPolicySimulator>` regardless of the `key` prop, whereas `IamRolesView` has no equivalent state that goes falsy on every tier switch, making `key={selectedTier}` the ONLY thing preventing reuse there. Verified directly: temporarily removing `key={selectedPolicyId}` alone did NOT make the new test fail (confirming `setDetail(null)` alone already prevents the stale-results bug for this view); removing `setDetail(null)` DID make it fail (confirming that's the real protection). Both the `key` prop and this finding's more precise framing are retained -- the `key` is correct, low-cost, defensive redundancy, just not this view's sole protection the way the original finding implied. |
| L3-H7 | UI | `.iam-tier-selector` has no wrap/scroll and `overflow: hidden` — with `MAX_CUSTOM_TIERS = 20` (plus 5 built-in), up to 25 tier buttons can silently clip off-screen with no visual indication or way to reach them | HIGH | **Fixed** — `flex-wrap: wrap` added to `.iam-tier-selector`. **CORRECTION (second re-audit): the original "verified via the E2E visual checklist (§13)" claim was FALSE — no §13 exists in this document, and no E2E/dune-dev deploy has actually occurred yet (see the dune-dev live-safety blockers below). The CSS fix itself is real and committed; the "verified via E2E" claim about it was not.** |
| L3-H8 | Network | Confirmed real, current dune-dev console port is `9088` and this is what the live Cloudflare tunnel actually routes to — the Network hat's report characterized the account README as having a stale `8088` entry for this hostname, but direct re-verification against the README found it already correctly states `9088` (corrected there on 2026-08-17, before this session). No doc defect existed in the README; the discrepancy traced to this dispatching session's own informal port reference made ahead of that audit, not the README itself | Confirmed non-issue (per Requirement 12: verified directly rather than trusting the sub-agent's own characterization) | **No fix needed** — the substantive, still-true finding is the restart's real ~5-15s live-outage window on `console-dev.darkdante.org`, which stands regardless of which port number was cited. **The real E2E deploy this row anticipates has NOT yet occurred as of this revision** — see the dune-dev E2E prerequisites (no backup of `runtime/generated/`/`runtime/secrets/` exists yet; the unexplained 06:59 UTC console-container restart is not yet root-caused) before it can proceed. |
| L3-M1 | Architect | `PolicySummary.statements`/`.defaultVersionId` are computed and sent on every catalog load but never consumed by the frontend (which independently re-fetches full detail) — dead payload weight, distinct from the already-tracked `versionCount` gap | MEDIUM | **Acknowledged, merged into the existing L2-M3 follow-up on #335** (UI hat's own re-assessment found L2-M3/L2-M4 compound into one interaction-cost problem, not two independent ones — see L3-M4 below) |
| L3-M2 | Security | `audit()`'s `appendFileSync(..., {mode:0o600})` does not retroactively correct a pre-existing, more-permissively-created audit log file's mode (pre-existing gap, predates this diff, but this diff's 9 new `iam.*` audit calls expand its practical blast radius onto a new security-critical surface) | MEDIUM | **Acknowledged, tracked as a new follow-up issue** — pre-existing, cross-cutting (affects every `audit()` call, not just IAM's), out of scope to fix as part of this PR |
| L3-M3 | Security | A Discord-handoff tier-name label (bot-side `guild_roles` mapping, still unimplemented per `docs/security/console-rbac-implementation-and-testing.md` Phase 3) can silently collide with a later-created, unrelated console-only custom tier of the same name, since the two systems share an uncoordinated string namespace by design | MEDIUM | **Acknowledged, documented** — added an explicit operational-risk note to `docs/console-iam.md`; full resolution depends on the still-unimplemented Phase 3 bot-side work and is out of scope here |
| L3-M4 | UI | Holistic re-assessment of L2-M3 (no version count in Policies list) + L2-M4 (attach-picker accessibility gap): these compound into one real two-view round-trip cost for an operator managing several policies from the Roles view's attach picker, not two independent polish items | MEDIUM | **Acknowledged** — recommend merging into a single follow-up issue on #335 rather than tracking separately (actioned as part of the #335 update in this session) |
| L3-M5 | UI | No "unsaved changes" warning exists anywhere in this diff (or the wider console codebase) when navigating away from an in-progress JSON/builder edit — a pre-existing, codebase-wide gap, but the new two-view + tab-level navigation surface widens the loss window compared to the single-editor predecessor | MEDIUM | **Acknowledged, not fixed** — consistent with existing codebase convention (confirmed via `MapsPanel.tsx`'s comparable dirty-tracking, which also never warns on navigation); tracked as a follow-up, not specific to this feature |
| L3-M6 | GRC | Risk classification "High" re-assessed independently: still defensible, but noted to sit at the ceiling given (a) the concurrency-safety guarantee rests on an unenforced code comment, not a CI-level guard, and (b) an incident-class vulnerability (`Object.prototype` pollution) existed in this same diff before being caught pre-merge | MEDIUM (process observation) | **Acknowledged** — classification kept at High; noted for re-assessment if `applyMutation()`'s synchronicity invariant is ever touched by unrelated future work |
| L3-M7 | GRC | The framing of this feature as "E2E-tested" overstates actual depth — all backend/frontend tests to date are unit/component-level; no browser/E2E test against a real dune-dev deploy has occurred | MEDIUM | **NOT resolved. CORRECTION (second re-audit): the prior "Resolved by this same work session's Phase 4... see §13" claim was FALSE — no §13, no Phase 4 commit, and no dune-dev deploy has occurred in this branch's history. Still open, blocked on the same dune-dev prerequisites noted at L3-H8 (no backup exists yet; unexplained 06:59 UTC restart not yet root-caused).** |
| L3-L1 | DBA | `migrateIfLegacyShape()`'s `null`-return trigger conditions were slightly undersold by its own comment (a non-alien object simply missing an `"owner"` key also hits this path) | LOW | Already fixed in passing at L2 (L2-L1); re-confirmed, no further action |
| L3-L2 | UI | Two visually-different "success green" shades used inconsistently between `IamPermissionGrid`'s checkboxes (real `--success` var) and the Test tab's result table (phantom `--success-color` var, both resolving only to hardcoded fallbacks since neither custom property is ever defined in `:root`) — a pre-existing, codebase-wide CSS-variable convention gap, with one new internal inconsistency introduced by this diff | LOW | **Acknowledged, not fixed** — cosmetic; to be confirmed by a human in a real browser once the actual E2E deploy happens (no §13/visual checklist currently exists in this document) |
| L3-I1–I4 | Architect, Network, Cloud Security | Confirmed clean, no action needed: `handoff.js`↔`policy.js` import graph (no cycle); migration-vs-handoff-login race window (structurally impossible given full synchronicity); `rbacParity.test.js` extractor correctly sees all 11 new routes (independently re-verified, not just re-trusted); CSS class collisions (none found); CSRF enforcement on the novel `DELETE .../versions/{vid}` two-path-param route shape (confirmed via live HTTP, with/without token); `actorTierIsOwner`'s own code comments correctly and honestly describe it as non-authoritative | — | No finding — explicitly confirmed clean by direct execution, recorded for completeness per this document's own evidence-first convention |

**Status as of this revision: every CRITICAL and HIGH finding across all three Layer 3 passes is now resolved, except L3-M7 (real dune-dev E2E), deferred by explicit operator scope decision, not left as an oversight.** L3-C1-R (cross-tier neutering), L3-C2 (`detachPolicy()` missing `actorTier` wiring), and L3-C3 (unbounded-regex DoS, now load-bearing) are all fixed with real engine-level changes. L3-H1a (nav-gate dead-end) and L3-H1b (component-gate) are both fixed. L3-H4 (DBA's 6 tests) and L3-H5 (Requirement 26 fixture) are both genuinely implemented in this revision — both had previously been falsely marked "Fixed" in an earlier revision; that false claim is corrected in place above, and this revision is the first one where the claim is actually true, independently verified via revert-and-reconfirm testing. L3-H6's `IamPoliciesView` test-coverage gap is closed, with a more precise finding about what actually protects that view documented in its row above. L3-M7 (real E2E against dune-dev) remains open, blocked on pre-existing prerequisites unrelated to this session's code changes (a missing backup, an unexplained restart) -- this is the one and only finding this revision does not resolve, by explicit user instruction ("ignoring E2E dune-dev, resolve all open items").

**Evidence-format correction**: the previous version of this paragraph claimed committed evidence files exist under `docs/design/evidence/console-custom-iam-roles/` — verified directly that this directory does not exist anywhere in this branch. No such evidence has been committed. This was itself a second, independent instance of the exact false-completion-claim class this document's own §10/§11 arithmetic corrections (L2-M1, L3-H2) already caught twice before — now caught a third time, in this same document, by the second Layer 3 re-audit. The concurrent-HTTP stress-test description (50 concurrent policy-create + 20 concurrent same-tier attach requests, 8 runs) may describe real work performed during a prior session, but its retention as a committed evidence file is unconfirmed and should not be assumed true without direct verification.

---

## 12. STRIDE report (Requirement 20, added retroactively for all three layers)

**Added per the corrected Requirement 20 text** (this account's README/AGENTS.md, updated 2026-08-18, issue #31/#35/#37): a STRIDE report is now a mandatory second output of every audit layer, alongside the per-hat findings register in §9/§10/§11 above. No STRIDE mapping existed anywhere for this feature before this section — a live compliance gap, corrected here rather than deferred. Per the corrected rule text, a finding that doesn't map to any STRIDE category is marked "N/A" explicitly rather than omitted, and this table does not replace §9/§10/§11 — it is a second, parallel view over the same findings.

### Layer 1 (design)

| STRIDE category | Finding(s) | Severity | Status |
|---|---|---|---|
| Spoofing | N/A — no findings in this layer concern identity/authentication spoofing | — | — |
| Tampering | L1-C1 (owner-lockout guard bypass via untouched-`tiers.owner` routes — a form of unauthorized state tampering bypassing an intended control), L1-C3 (dangling `attached[]` references — referential-integrity tampering risk) | CRITICAL, CRITICAL | Both fixed |
| Repudiation | N/A — no findings in this layer concern audit-trail/non-repudiation gaps | — | — |
| Information Disclosure | L1-L1 (owner-pasted JSON has no content-based secret-embedding validation) | LOW | Accepted, not fixed (pre-existing, out of scope) |
| Denial of Service | L1-L3 (`matchAction()`'s unescaped regex construction, flagged here as a blast-radius amplification risk, not yet load-bearing at this layer) | LOW-MEDIUM | Accepted for L2 tracking at the time; **superseded by L3-C3 below, now fixed** |
| Elevation of Privilege | L1-C2 (fail-open aggregation could silently grant a Deny-intended action) | CRITICAL | Fixed |
| Not STRIDE-mappable | L1-H1 (concurrency control), L1-H2 (route-extractor parsing gap), L1-H3 (Requirement 26 partial), L1-H4 (missing findings register), L1-H5 (migration whole-file-reject), L1-H6 (test plan not implementable as written), L1-H7 (no frontend error surfacing), all L1-M*/L1-L2 rows | Various | These are architecture/process/QA/UI findings without a clean Confidentiality-Integrity-Availability-adjacent threat framing — correctly excluded per the corrected rule's own guidance that not every finding maps to STRIDE |
...
|---|---|---|---|
| Spoofing | N/A | — | — |
| Tampering | L2-C1 (checkbox grid grouped over the wrong object, could not correctly reflect or change real grant state), L2-C2 (delete-confirmation state not scoped to selection — could arm a delete against the wrong object), L2-C3 (`Object.prototype` pollution — the most severe tampering finding in this entire feature, corrupting global process state) | CRITICAL, CRITICAL, CRITICAL | All fixed |
| Repudiation | N/A — no findings in this layer concern audit-log integrity or non-repudiation | — | — |
| Information Disclosure | N/A — no findings in this layer concern unintended data exposure | — | — |
| Denial of Service | N/A — no findings in this layer independently identified a DoS vector (the regex-ReDoS risk was tracked at L1/L3, not re-raised here) | — | — |
| Elevation of Privilege | L2-C3 (`Object.prototype` pollution also has a direct EoP angle: a `settings:write` session could use it to corrupt shared state affecting every tier's evaluation, not just its own) | CRITICAL | Fixed (same fix as the Tampering row above — one finding, two STRIDE angles) |
| Not STRIDE-mappable | L2-C4 (tautological concurrency test), L2-H1 (shallow-copy bug), L2-H2 (UI retry/loading-state gaps), L2-H3 (missing test coverage), L2-H4 (version-ID collision, uncommitted verification), L2-H5 (missing 0o600 test coverage), L2-H6/L2-H7 (stale PR/doc claims), all L2-M*/L2-L* rows | Various | QA/DBA/GRC/UI findings without a clean STRIDE angle |

### Layer 3 (integration) — both passes

| STRIDE category | Finding(s) | Severity | Status |
|---|---|---|---|
| Spoofing | N/A | — | — |
| Tampering | L3-C1-R (cross-tier neutering/stranger-grant — a settings:write-only tier silently modifying an unrelated tier's permission set, including a built-in tier), L3-C2 (`detachPolicy()` had no privilege-ceiling wiring at all, an unmitigated tampering vector against any tier's attached-policy list) | CRITICAL, CRITICAL | Both fixed (second re-audit) |
| Repudiation | L3-M2 (audit-log file mode not retroactively corrected — a repudiation-adjacent gap, since an under-permissioned audit log is more exposed to tampering/deletion, undermining its value as a non-repudiation record) | MEDIUM | Acknowledged, tracked as a follow-up issue (pre-existing, cross-cutting) |
| Information Disclosure | L3-M3 (Discord-handoff tier-name collision could cause a Discord user to silently inherit an unrelated console tier's grants — an unintended-disclosure-of-access risk, not classic data exposure, but the closest-fitting category) | MEDIUM | Acknowledged, documented in `docs/console-iam.md`; full resolution depends on unimplemented Phase 3 bot-side work |
| Denial of Service | L3-C3 (`matchAction()`'s unbounded regex construction, now load-bearing for the privilege-ceiling check — catastrophic backtracking could hang the entire process) | CRITICAL | **Fixed** this session — replaced with a backtracking-free glob matcher, independently confirmed via `semgrep --config auto` (0 findings after) and a direct before/after timing demonstration (old: hung indefinitely under a 10s external timeout; new: 0.57ms) |
| Elevation of Privilege | L3-C1 / L3-C1-R (the original and then-corrected privilege-ceiling bypass — a `settings:write`-only tier reaching owner-equivalent access), L3-C2 (`detachPolicy()` EoP angle: stripping an unrelated tier's restriction-granting policy could, in principle, be paired with other calls to widen an actor's own effective reach depending on shared-policy topology) | CRITICAL, CRITICAL | Both fixed |
| Not STRIDE-mappable | L3-H1a/L3-H1b (UI nav/component gates, both fixed), L3-H2/L3-H3 (GRC arithmetic/doc-completeness corrections, fixed), L3-H4/L3-H5 (DBA test/fixture gaps, fixed this revision after an earlier false "Fixed" claim), L3-H6 (stale Policy Simulator results, test-coverage gap closed this revision), L3-H7 (CSS overflow, fixed), L3-H8 (Network port confirmation, non-issue), L3-M1/L3-M4/L3-M5/L3-M6 (dead payload, UI interaction cost, no-unsaved-changes-warning, risk-ceiling process note), L3-M7 (no real E2E has occurred, deferred by explicit operator scope decision), L3-L1/L3-L2 (minor DBA/UI items), L3-I1–I4 (confirmed-clean items) | Various | Architecture/GRC/UI/QA/Network findings without a clean STRIDE angle, or (L3-H4/H5/M7) DBA/GRC findings about missing test coverage and missing E2E validation rather than a threat-model gap itself |

**Summary**: every CRITICAL finding across all three layers maps to Tampering and/or Elevation of Privilege (consistent with this feature's nature — an access-control engine, where the dominant threat class is exactly "can a less-privileged actor change state or reach access it shouldn't"), with one CRITICAL Denial of Service finding (L3-C3) found and fixed only at Layer 3 once a Layer 2 addition made a pre-existing weakness load-bearing. No Spoofing findings exist anywhere in this feature's three layers — expected, since this feature does not introduce any new authentication mechanism; it operates entirely downstream of an already-authenticated session's tier. This asymmetry (zero Spoofing, zero Repudiation-as-CRITICAL, heavy Tampering/EoP concentration) is itself a useful, honest signal about where this specific feature's real risk actually concentrates, and should inform where future review effort on this feature is spent.
</content>
