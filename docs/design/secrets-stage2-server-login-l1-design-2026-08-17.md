...
## 10. Layer 2 Implementation Audit — Findings Register

Dispatched 2026-08-17 as 3 independent Task-tool worker groups (covering all 8 hats) against the actual implemented code on `feat/281-secrets-library` (commit `955954b8`, post-Layer-1-fixes). This layer exists specifically to catch implementation-level bugs a prose-only design review cannot — and it did.

| # | Hat | Severity | Finding | Resolution |
|---|---|---|---|---|
| 1 | Security Architect | **CRITICAL** | `dune_secrets_read_secret()` (the library primitive both the resolver and `secrets-cli.sh` delegate the real read to) only checked `.enc`-file readability for its own fail-closed decision — it had no knowledge of the migration marker at all. The outer callers (`_resolve_stage2_secret`, `_dune_secrets_stage2_state`) correctly implemented the belt-and-suspenders `.enc`-or-marker check per §2/§3's `[R2]` fixes, but then delegated the *actual read* to a function with a narrower, stale contract — so a `.enc` file that existed but was unreadable (permission drift) caused `dune_secrets_read_secret` to silently fall back to legacy and return success, even though the outer check had correctly identified the secret as migrated via the marker. Live-reproduced: `dune secrets verify` printed `OK` against a `chmod 000`'d `.enc` file, and `cleanup-legacy` — the one destructive command whose entire safety property rests on "re-verify immediately before deleting" — would have deleted the last good (legacy) copy while the real `.enc` file sat there broken. | **Fixed at the source**: `dune_secrets_read_secret()` itself now checks both signals (the `.enc` file OR the marker, `-r` not `-e`), making it the single source of truth every caller relies on instead of each reimplementing the same check independently — which is exactly how the two copies drifted out of sync in the first place. `runtime-env.sh`'s `_resolve_stage2_secret` simplified accordingly (removed its own duplicate enc/marker check, now just calls `dune_secrets_read_secret` and trusts its result). New adversarial regression test added: `runtime/tests/test-secrets-stage2.sh` Test 7b, which `chmod 000`'s a real `.enc` file (marker still present and readable) and asserts both `verify` and `cleanup-legacy` correctly fail closed. Mutation-tested: reverting the marker-check fix causes Test 7b to fail exactly as expected. |
| 2 | Software Architect / QA | **HIGH** | Zero test coverage existed for the permission-drift scenario Finding 1 (CRITICAL) describes — mutating away the `-r`-vs-`-e` fix and the marker-fallback logic both left the full test suite green. | Resolved by the same Test 7b added for Finding 1 — mutation-tested to confirm it now catches both regressions (reverting either the `-r` predicate or the marker-check causes Test 7b to fail). |
| 3 | (self-discovered while fixing #1) | **CRITICAL** | While implementing the Finding-1 fix in `_resolve_stage2_secret`, a second, independent bug was found: `if value="$(cmd)"; then ...; fi` followed by a bare `rc=$?` on the next line does **not** capture the command substitution's exit status on the false branch — bash's `$?` after a completed `if` statement reflects the last command actually *executed* inside whichever branch ran, and when the `then` block's body never executes (false condition), nothing runs, so `$?` becomes bash's default-empty-branch value of 0. This silently discarded every real failure from `dune_secrets_read_secret` and made the entire "hard stop once migrated" mechanism (the design's own central safety property, §2) a complete no-op that always returned success regardless of the underlying failure. Live-reproduced independently of Finding 1: with a correctly-detected wrong-identity failure and the correct error message printed to stderr, the resolver still returned exit 0. | **Fixed**: replaced the `if value="$(...)"; then ... fi; rc=$?` pattern with `value="$(...)" \|\| rc=$?` (capturing the exit status directly on the assignment statement, unambiguous regardless of branch execution). Verified via a clean-environment (`env -i`) reproduction before and after the fix: exit 1 (correct) after, exit 0 (bug) before. Mutation-tested: reverting to the buggy pattern causes `test-secrets-stage2.sh` Test 3 to fail exactly as expected — Test 3 already existed (from the Layer 1 deliverables) and turned out to be the test that would have caught this, once the code path it exercises was actually correct enough to reach the buggy line. |
| 4 | Software Architect | MEDIUM | `dune secrets migrate --dry-run <name>` (flag before name) failed with a misleading "unknown option" error naming the *secret name* as the unrecognized option — only `migrate <name> --dry-run` (flag after name) worked, an undocumented positional restriction most CLI tools don't impose. | Resolved: `cmd_migrate`'s argument parsing rewritten to accept `--dry-run` in either position via a single pass over all arguments, tracking the one non-flag argument as `<name>` regardless of where it appears. Verified both orderings work identically. |
| 5 | Software Architect | MEDIUM | Genuine code duplication between `runtime-env.sh`'s migration-state check and `secrets-cli.sh`'s `_dune_secrets_stage2_state` — both independently re-derived the same enc-or-marker logic, meaning Finding 1's fix would otherwise have needed to be applied in two places by hand, with no structural guarantee they'd stay in sync. | Substantially resolved by Finding 1's fix itself: `runtime-env.sh` no longer duplicates the check at all (delegates entirely to `dune_secrets_read_secret`). `secrets-cli.sh`'s `_dune_secrets_stage2_state` still exists as a separate function, but only because `status`'s 4-state display (`not-migrated`/`migrated`/`broken`/`backend-not-configured`) needs more granularity than `dune_secrets_read_secret`'s binary success/failure alone provides — this remaining duplication is a display-layer concern, not a safety-critical one, since `verify`/`cleanup-legacy`'s actual correctness-determining calls go through `dune_secrets_read_secret` directly, not through `_dune_secrets_stage2_state`. |
| 6 | UI Design/Architect | LOW | `secrets-cli.sh`'s out-of-scope rejection message (`migrate`/`cleanup-legacy`) cited the internal design-audit doc (`docs/design/secrets-stage2-...md`) as if it were operator-facing documentation, rather than the real operator doc `status` already correctly points at (`docs/security/secrets-management.md`). | Resolved: rejection message now cites `docs/security/secrets-management.md`, consistent with `status`'s own citation. |
| — | Multiple hats | — (confirmed correct, no fix needed) | CRITICAL allow-list fix (§3), argv/process-exposure constraint (§3), `--dry-run` filesystem-no-op guarantee, standalone `verify` command's non-destructiveness, atomic write-ordering (DBA), zero new network/cloud-provider surface (Network/Cloud Security hats) — all independently re-verified against the real implementation and real command output, all confirmed correctly implemented exactly as designed. | No action needed; documented here as evidence per Requirement 20's evidence-first standard, not silently assumed. |

**Verification after all L2 fixes**: full `runtime/tests/test-secrets-stage2.sh` (11 assertions, including the new Test 7b) passes both as root (Test 7b correctly self-skips, matching the same root-vs-non-root constraint already accepted for `test-secrets-lib.sh`'s own Test 8) and, independently confirmed via a real non-root user account created for this purpose, with Test 7b passing for real. `runtime/tests/test-secrets-lib.sh`, `tests/db-system-backup-test.sh`, and `shellcheck -S warning` across every changed file all re-verified green after the fixes. Two mutation tests performed against the two CRITICAL findings (Finding 1's marker-check, Finding 3's `$?`-capture pattern) independently confirmed the new/existing tests genuinely fail when either fix is reverted.

**Assessment**: this layer is precisely why Requirement 20 mandates a separate implementation audit distinct from the design review — both CRITICAL findings here (the marker-check-not-threaded-through bug, and the `$?`-after-`if` bash pitfall) were invisible to a prose-only Layer 1 review of the design doc's own pseudocode, because the design doc's code blocks were illustrative sketches, not the actual shipped implementation, and the second bug in particular is a well-known but easy-to-miss bash semantics trap that only surfaces when actually executed under a real failure condition.
