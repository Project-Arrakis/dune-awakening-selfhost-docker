...
- **Existing operators mid-setup on the old flow** when this ships: the old UI (independent OAuth app form, separate Add to Discord / Connect to hosted bot buttons) should remain available as a fallback (§9) precisely so nobody's in-progress setup breaks out from under them.
...
5. **Core:** `hostedBotAutoInvitePendingStates` + `/auto-invite/start` + `/auto-invite/complete` (including the `consoleUrl` https-scheme validation, issue #843 M1, and `reclaimed=true` handling, issue #838) + the new wizard step 1 UI (replacing/supplementing the current OAuth-app-config form + two-button flow) + the role-picker UI (widget TBD per §4.6, guild-confirmation banner per §4.3/issue #838).
...
Phases 1-4 can be built and tested independently of Core; phase 5 is the only phase that depends on all prior phases plus the operator's own Discord Developer Portal action.

## 9. Open decision: deprecate or keep the independent-OAuth-app path?

Not resolved in this design — flagged explicitly for the operator's own call, not assumed:

- **Option A — deprecate, keep as fallback.** New wizard step 1 defaults to the auto-invite flow; the old "configure your own Discord Application" form moves behind a collapsed "advanced / manual setup" disclosure, for the (currently unknown, possibly zero) case of an operator who wants to use a *different* Discord Application than Sahir Venn's for some reason.
- **Option B — remove outright** once the new flow ships and is confirmed working, simplifying the codebase (less surface, fewer states to test) but removing a working, already-shipped, already-tested path (PR #801) with no proven need to keep it.
- **Option C — keep both, permanently, operator's explicit choice at step 1** ("Use the shared hosted bot" vs. "Use my own Discord Application" — meaningful if there's ever a real reason an operator wouldn't want their guild's bot-invite consent screen to show "Sahir Venn," e.g. white-labeling).

Recommendation for discussion, not a decision: start with **Option A**. It's the lowest-risk choice (nothing already-shipped is deleted), and the "advanced" disclosure gives real signal (via usage, if this org ever adds that kind of telemetry, or just by asking) on whether Option B is ever safe to do later.
