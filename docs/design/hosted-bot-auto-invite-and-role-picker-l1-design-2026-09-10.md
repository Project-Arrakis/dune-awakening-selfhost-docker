...
sequenceDiagram
    ...
    M-->>ML: signed redirect (HMAC over consoleUrl|state|ok|guildName|exp,<br/>issue #845) -- /return?consoleUrl=...&state=...&ok=true&guildName=...&sig=...
...
- **New: `functions/api/consoles/auto-invite/return.js`** (not a proxy — a real, small Function with its own logic, the first of its kind in mentat-link for this feature). Receives mentat's **signed** internal redirect from the callback handler (§4.1), **verifies the HMAC signature over `consoleUrl|state|ok|guildName|exp`** using `MENTAT_PROXY_SHARED_SECRET` before doing anything else (issue #845 — a scheme-only check was itself a new, trivially-reachable open redirect), and renders a minimal `200 html` page that loads an external, same-origin script (`return.js`'s companion static asset, not an inline `<script>` — issue #846, avoids the `script-src 'self'` CSP conflict) reading `location.search` client-side to perform `window.location = "${consoleUrl}/api/integrations/discord/hosted-bot/auto-invite/complete?" + params`. `guildName` is HTML-escaped at every interpolation site (issue #846). This is the fix for issue #834 (never triggers `proxyRequest()`'s `TRUSTED_CROSS_ORIGIN_REDIRECTS` allowlist) made safe against its own round-2 regression (#845, an unauthenticated caller could otherwise hit this route directly).
...
- **Tampering:** `guildName` in the return-leg is cosmetic display text only; the authoritative `guildId`-to-owner binding happens server-side before any redirect is issued. The return-leg mechanism no longer relies on a `302 Location:` header crossing mentat-link's `TRUSTED_CROSS_ORIGIN_REDIRECTS` allowlist (issue #834). **New this round, resolved:** `guildName` (attacker-controlled Discord guild-name text) was found flowing into the bounce page and Core's return page with no stated escaping discipline (issue #846) — this codebase already learned this exact lesson once, in `hostedBotOAuthReturnPage()`; the bounce page now escapes every interpolation site and loads its logic as an external script rather than templating values into inline script text (also resolves a CSP `script-src 'self'` conflict the inline-script design would have hit). **New this round, resolved:** the role-picker write path (§4.7) had no transaction boundary around its read-check-write sequence, letting the new second caller race the old `/setup` portal handler and potentially defeat the tier-conflict separation-of-duties control entirely (issue #847) — fixed via an explicit transaction.
...
**Complete, two rounds, 2026-09-10.** All eight hats dispatched as genuinely independent agent workers (fresh context each, no shared assumptions) in each round, reading this document plus the real, current code in `dune-awakening-selfhost-docker`, `mentat`, and `mentat-link` directly, per Requirement 20. **Round 2 was not a formality — it re-verified round 1's own claimed fixes against real code rather than trusting this document, and found two round-1 "Resolved" CRITICALs (#833, #839) were not actually resolved, plus a new CRITICAL the round-1 fix itself introduced (#845).** This is exactly the scenario Requirement 20's "an internal audit's 'all findings resolved' claim is a hypothesis, not a fact, until independently confirmed" language exists for — documented here in full rather than quietly corrected, since a future reader (human or LLM) should be able to see that this document was wrong twice about its own worst finding, not just that it's currently right.

All CRITICAL and HIGH findings from both rounds are resolved in this revision (§2-§9 above). MEDIUM/LOW findings are either fixed inline or explicitly deferred with justification, per issues #843/#852.
...
### STRIDE report (both rounds combined)

| STRIDE category | Findings mapped | Severity | Resolution |
|---|---|---|---|
| Spoofing | #833, #839, #844, #845 | CRITICAL, HIGH, CRITICAL, CRITICAL | Resolved (§5) |
| Tampering | #834, #835, #836, #846, #847 | CRITICAL, CRITICAL, CRITICAL, HIGH, HIGH | Resolved (§5) |
...
| Elevation of Privilege | #833, #835, #839, #844, #847, #848 | CRITICAL, CRITICAL, HIGH, CRITICAL, HIGH, HIGH | Resolved (§5) |

Findings #837, #838, #841, #842, #849, #850, #851, and most of #843/#852's items do not map cleanly to a STRIDE category (contract-accuracy, UX-flow, operational-hygiene, and documentation findings respectively) — listed as N/A per Requirement 20's own guidance not to force a mapping that doesn't fit.
...
**A note for whoever picks up Layer 2:** the two rounds above are not a formality to skim past. The most consequential lesson isn't any single finding — it's that this document's own confident "Resolved" status on #833/#839 after round 1 was wrong, and would have shipped a genuinely exploitable session-fixation vulnerability had round 2 not been run. Treat every "Resolved" status here the same way round 2 treated round 1's: as a claim to independently re-verify against real code during the Layer 2 implementation audit, not as settled fact.
