...
| All recovery codes (TOTP still works) | `POST /api/auth/2fa/recovery-codes/regenerate` (shipped #512, 2026-08-26) issues a fresh set against the current password + a current TOTP code. **No settings-panel control calls it yet** (deferred with #515), so today this is reachable via the API but not from the console UI; the full host-filesystem reset below remains the UI-only operator's fallback. | Old sheet worthless the moment a new set is issued — intended |
...
**Corrected 2026-08-26: this paragraph previously described one unified visibility mechanism; the reality is two, one shipped and one not.** Whenever enrollment or re-setup completes, `settings.totp-setup`/`settings.totp-regenerated` audit entries fire — this was already real. Issue **#425** (backup/restore integrity) is now shipped separately: `secondFactorStore.js` tracks a monotonic `epoch` per write plus an independent watermark file, and on the next recovery-code use after a **restored-but-still-present** older file is detected, emits `auth.second-factor-reset-detected`, invalidates the entire code set rather than honoring a resurrected one, and logs a non-blocking startup banner. What is explicitly **not** built: detecting a **deleted** file that had prior enrollment activity in the retained audit history (the "no TOTP state where state previously existed" case this paragraph used to describe as if it existed) — tracked as a new, separate issue rather than left silently unimplemented. Partially built since: regenerating recovery codes outside an emergency now has a real route (#512, see §2.3), though not yet a settings-panel control (deferred with #515). Still not built: the deleted-file/audit-history detection variant for the total-loss case (tracked as #513, distinct code path from #425's watermark mechanism).
