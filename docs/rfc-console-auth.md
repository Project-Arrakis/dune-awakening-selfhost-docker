...
**Recovery-code regeneration outside an emergency** is specified as a standing settings action (same panel as rotation), requiring the same fresh proof of possession as rotation, invalidating the entire old set, and displaying the new set once behind the acknowledgment gate, emitting `settings.recovery-codes-regenerated`. **Shipped (#512 route, #515 settings control).** `POST /api/auth/2fa/recovery-codes/regenerate` requires the current password plus a current TOTP code, sits behind the credential-proof limiter (separate from the login limiter since #521/#522 — sharing it let a stolen session cookie lock every operator out of sign-in), invalidates the entire old set, emits `settings.recovery-codes-regenerated` with the acting principal and a `healedRollback` flag, and returns the new set once behind a `no-store` cache directive. It is mapped to the `settings:regenerate-recovery-codes` action, so the default `admin` policy's explicit `Deny settings:*` keeps it owner-only **on an unedited policy store** — that is a default, not an enforced property (#529). It is excluded from `ENROLL_ALLOWED`, and the handler calls `requireAuth` itself rather than relying on where it is registered (#519). Unlike password rotation it revokes no sibling sessions — rotating a recovery-code sheet is not a login-credential change. The Settings panel carries a Two-Factor Authentication section (visible only when a factor is enrolled) that drives it. The enrollment screen and that section share a `RecoveryCodesPanel` component, which shares **the checkbox label and the disabled-until-acknowledged gate** — the heading and intro copy are per-call-site props and do legitimately differ, so this is not a guarantee that the two can never describe once-only codes differently (corrected: an earlier revision of this paragraph claimed exactly that).
