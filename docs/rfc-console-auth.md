...
**Popup+poll sign-in (#574).** *Sign in with Discord* on the main sign-in page opens a **popup window** instead of navigating the whole tab, and the main page polls `GET /api/auth/state` until the popup completes — the console never itself reloads. This is presentation only: the popup runs the exact same `/api/auth/discord/start` → Discord → `/api/auth/discord/callback` round trip (same PKCE, same state cookie, same tier resolution) as the full-page flow always has, tagged internally with `presentation: popup` so the callback knows to serve a small self-closing page instead of redirecting the popup to the full console. A plain left-click on the button triggers the popup; a modifier-click (ctrl/cmd/middle-click) is left alone and opens today's full-page flow in a new tab as the browser's own new-tab intent would suggest. If the browser blocks the popup, the same click falls back automatically to the full-page redirect — no second interaction required. This is the only entry point affected: the guided *setup* flow above (`purpose: setup`) is unchanged and does not use the popup presentation.
