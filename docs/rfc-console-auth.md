...
4. **Confirm with the admin password again** (`POST /api/setup/discord-finalize`) — fresh proof, the same rule §2.3 applies to rotating the password, since enabling a sign-in method is a credential change. The console writes the same keys as the settings API and reports that a restart is required; after restart the sign-in page shows *Sign in with Discord* as the primary action with *Use the admin password instead* beneath it as the break-glass path.
