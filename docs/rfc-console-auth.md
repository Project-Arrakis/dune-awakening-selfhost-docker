...
Unset by default — byte-identical behavior to today; no passkey routes are even registered. When set, this string becomes the WebAuthn Relying Party ID, and the operator setting it **is** the confirmation that a secure context (HTTPS, or literal `localhost`) exists somewhere in their access path — WebAuthn's browser APIs (`navigator.credentials.create()`/`.get()`) are unavailable outside a secure context regardless of anything server-side. The frontend must feature-detect this state (`window.isSecureContext` / `PublicKeyCredential` presence) rather than letting the raw browser API fail: on a non-secure context with `WEBAUTHN_RP_ID` set, the settings panel shows registration as unavailable with an honest explanation ("passkeys require HTTPS or localhost; you are connected over plain HTTP"), and the login form's passkey branch is hidden or annotated the same way. There is no security consequence to the misconfiguration either way — only the difference between an explained limitation and an unexplained dead button.
...
- **Consumption** goes through the same serialized atomic-write queue as the passkey store, and the recovery endpoint sits behind the existing login limiter (8 attempts/key, 32 global, 15-minute block).
...
2. A recovery login requires the **password plus one unused recovery code**. The code substitutes for the TOTP factor only — never for the password. A stolen recovery-code sheet alone does not log in.
3. A successful recovery login creates a **restricted re-setup session** with the same access set as §4's enrollment-only session (TOTP setup, confirmation, recovery-code display, logout — nothing else). It is not a normal owner session with a redirect; nothing outside re-setup is reachable until re-setup completes.
4. Completing re-setup regenerates the TOTP secret (fresh QR + text secret), generates a **fresh set of 10 recovery codes** (the remaining unused codes from the old set are invalidated — whoever holds the old sheet holds nothing), displays them once behind the same acknowledgment gate as §4, invalidates the re-setup session, and requires a normal password+TOTP login.
...
- **Tier 3's hardening is not gated behind an opt-in**. On first password authentication after upgrade, an installation with no TOTP state receives a **10-minute, non-renewable, enrollment-only session**. That session can access only TOTP setup, confirmation, recovery-code display/download, and logout; any other console request from it receives a redirect to the enrollment screen (not a bare 403), with one line of explanation: a security upgrade requires two-factor setup before continuing. The login screen itself carries the same line post-upgrade, so login leading into setup is explained, not surprising. Successful TOTP confirmation atomically creates the `local-owner` second-factor state, then displays the recovery codes once — behind an explicit **"I have saved these codes" acknowledgment** that must be given before the display can be dismissed — then invalidates the enrollment session and requires a normal password+TOTP login. (The confirmation code just used cannot be reused for that login — TOTP single-use within a step is standard — and the transition screen says to wait for the authenticator's next code rather than leaving the immediate rejection unexplained.) Existing ordinary sessions are in-memory and disappear when the upgraded console process starts. This makes "mandatory" true immediately without locking out an operator before enrollment.
...
| Unit — passkey ceremonies | Registration/authentication success and failure paths, using `@simplewebauthn/server`'s own known-good test fixtures; fresh proof-of-possession required before registration; ceremony endpoints observe the login limiter; zero-egress proof with `globalThis.fetch` stubbed to throw | new `console/api/test/passkeyCeremonies.test.js` |
| Unit — signCount safety | Concurrent-request queueing behavior; replay/rollback detection; counter-reset action requires fresh proof and emits its audit event | new `console/api/test/passkeySignCount.test.js` |
...
| Integration — recovery login | Password+code (never code alone) → restricted re-setup session (nothing outside re-setup reachable) → forced TOTP re-enrollment → old codes dead, new set issued once | extends `auth.test.js` |
...
| Upgrade-path | Fresh and existing password installs receive only an enrollment session (10-minute TTL, blocked requests redirect to enrollment) until TOTP confirmation; interrupted enrollment restarts with a regenerated secret; post-confirmation password-only login is rejected; recovery-code display requires acknowledgment; `WEBAUTHN_RP_ID` configured mid-lifecycle enables `local-owner` passkeys without inventing individual identities | extends existing upgrade-path test conventions |
...
Three design iterations preceded this RFC, each corrected by direct Layer 1 audit findings (a Discord-primary-plus-Cloudflare-Access design, then a passkey-sole-primary design) before arriving at the layered model presented here. This document presents the resulting design as revised by upstream review and post-merge validation; the fork's issues #331 (design lineage), #398 (post-merge findings), and #357 (documentation reconciliation) hold the retrievable history.
