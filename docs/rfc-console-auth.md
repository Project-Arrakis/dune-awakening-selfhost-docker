...
**Recovery-code regeneration outside an emergency** is specified as a standing settings action (same panel as rotation), requiring the same fresh proof of possession as rotation, invalidating the entire old set, and displaying the new set once behind the acknowledgment gate, emitting `settings.recovery-codes-regenerated`. **Not yet wired (corrected 2026-08-26, tracked as #512):** `secondFactorStore.js`'s `regenerateRecoveryCodes()` implements the storage half; no HTTP route or settings-panel control calls it. Until it ships, an operator who has all recovery codes but still holds their authenticator has no way to get a fresh set except the full host-filesystem reset in §3.4.
...
| All recovery codes (TOTP still works) | `secondFactorStore.js`'s `regenerateRecoveryCodes()` exists and does this, but **no HTTP route or settings-panel control calls it yet** (corrected 2026-08-26 — this row previously described it as a working settings action). Until that route ships, the only way back is the full host-filesystem reset below. | Old sheet worthless once wired — intended |
...
**Corrected 2026-08-26: this paragraph previously described one unified visibility mechanism; the reality is two, one shipped and one not.** Whenever enrollment or re-setup completes, `settings.totp-setup`/`settings.totp-regenerated` audit entries fire — this was already real. Issue **#425** (backup/restore integrity) is now shipped separately: `secondFactorStore.js` tracks a monotonic `epoch` per write plus an independent watermark file, and on the next recovery-code use after a **restored-but-still-present** older file is detected, emits `auth.second-factor-reset-detected`, invalidates the entire code set rather than honoring a resurrected one, and logs a non-blocking startup banner. What is explicitly **not** built: detecting a **deleted** file that had prior enrollment activity in the retained audit history (the "no TOTP state where state previously existed" case this paragraph used to describe as if it existed) — tracked as a new, separate issue rather than left silently unimplemented. Also not yet built: the settings-panel action to regenerate recovery codes outside an emergency (§3.4's own table above corrects this; tracked as #512) and the deleted-file/audit-history detection variant for the total-loss case (tracked as #513, distinct code path from #425's watermark mechanism).
