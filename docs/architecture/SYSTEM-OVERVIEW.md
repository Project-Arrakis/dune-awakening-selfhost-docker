...
Audience: engineers working on this fork's code (Core, the console, the
...
All facts below were verified against `origin/main` (commit `235b02cc`,
`VERSION` = `v1.3.90`) by reading the actual source files cited. This is a
fork of [`Red-Blink/dune-awakening-selfhost-docker`](https://github.com/Red-Blink/dune-awakening-selfhost-docker);
this fork does not own the `VERSION` numbering (see
[`CHANGELOG.md`](../../CHANGELOG.md) for the versioning convention).
...
the opt-in metrics and public-probe stacks also define named Compose
services in their own Compose projects. The actual game-server processes
(Postgres, RabbitMQ, the Director, the Gateway, and every dynamic map
partition) are started as raw `docker run` containers by
`runtime/scripts/start-all.sh` and related scripts, not by Compose. Every
container this repo starts (Compose-managed or raw
`docker run`) carries a `com.docker.compose.project` label
(`runtime/tests/test-container-compose-labels.sh`) so all containers stay
attributable to one resolved project name regardless of how they were
started.
...
project (`${DUNE_COMPOSE_PROJECT_NAME}-metrics`) via `dune metrics`. Six
services: `dune-prometheus`, `dune-node-exporter`, `dune-cadvisor`,
`dune-postgres-exporter`, `dune-alertmanager` (fails closed if
`runtime/secrets/alert-relay-token.txt` is missing — no accidental
unauthenticated Discord relay), `dune-grafana`. Prometheus and Grafana
publish only on `127.0.0.1` by default. See
...
- **Test framework:** Node's built-in `node --test` (`npm test`), 103 test
...
- **Key source modules** (non-exhaustive; see the file for the full list):
  ...
  (30 files) and an `integrations/discord/` directory (17 files, the
...
- **Structure:** `src/api/` (typed HTTP client modules, one per backend
  ...
  `addons`, `adminTools`, `backups`, `bases`, `blueprints`, `carePackage`,
  `database`, `exchange`, `guilds`, `landsraad`, `liveMap`, `logs`,
  `maps`, `players`, `server`, `services`, `settings`, `storage`,
  `updates`, `vehicles`), `src/components/` (shared UI), `src/lib/`
  (utilities).
...
- **`dune`** — the game-world schema, populated primarily by the
  ...
  `dune.player_state`, `dune.landsraad_*`, world-partition tables, etc.).
  A small number of console-authored tables also live in this schema for
  historical reasons (e.g. `dune.console_player_playtime`).
- **`console`** — console-owned adapter state, e.g.
  `console.discord_player_links`, `console.discord_account_links`. A
  migration inside `duneDb.js` moved the Discord link tables from `dune.*`
  to `console.*` non-destructively (old rows are copied, never dropped,
  so no operator loses link state on upgrade).

Both schemas are covered by the same `pg_dump`-based backup pipeline (see
[`docs/console/database-backups.md`](../console/database-backups.md)) —
there is no separate backup mechanism for console-owned tables.
...
`storage`, `network`, `db`, `db-manage`/`database`, `config`/`server`,
`memory`, `memory-swap`, `sietches`, `maps`, `deepdesert`, `admin`), each
delegating to a dedicated script under `runtime/scripts/`. Run `dune help`
for the exact, current usage text — it is generated from the same source
this document was verified against and will not drift from it the way a
duplicated list in this document eventually would.
...
orphaning an operator's existing game-data volumes. See the root
`AGENTS.md`-equivalent context for this account: this exact mechanism is
why `dune-awakening-selfhost-docker`'s Compose files deliberately do not
hardcode a project `name:` (with the sole, narrow exception of
`docker-compose.web.yml`, which fixes its own project name independently
of the main stack's resolved name).
...
|---|---|---|
| `runtime/secrets/` | Yes | Operator credentials: Funcom Self-Host Service Token, FLS API key, RabbitMQ HTTP token-auth secret, command-auth token, the console's own auto-generated admin password, the Discord adapter/actor/handoff secrets, Grafana admin password, the alert-relay bearer token. Created empty by `dune init`. |
...
1. **The Discord adapter** (`console/api/src/integrations/discord/`,
   17 files) — a built-in, disabled-by-default, bearer-token-protected
   ...
   `broadcastProvider.js`, `commandCatalog.js`). This is the
   **operator-facing, current** integration — see
...
**not-yet-fully-shipped** write-command architecture for Discord
(two-phase preview/execute with nonces) — treat that document as a design
plan with open issues (#215-223 at time of writing), not as a description
of what is live in `main` today.
...
for `console/api`, and a `release-gate` job that requires all seven other
CI jobs to succeed. See [`docs/security/`](../security/) for the individual
...
- [`docs/console/API-REFERENCE.md`](../console/API-REFERENCE.md) — full HTTP endpoint reference (~314 documented routes).
...
- [`docs/security/secrets-management.md`](../security/secrets-management.md) — current secrets inventory and handling.
