...
| Diagnose stale systemd project paths | **Implemented in v1.3.65 and refined afterward.** `dune update auto status` and `dune doctor` distinguish a legacy `dune update --yes` service in the current checkout from an `ExecStart` that genuinely points elsewhere. The warning includes a settings-preserving repair command, and re-enabling auto updates rewrites the unit to use the current policy runner. |
| Recover root-owned auto-update state | **Implemented after v1.3.65.** Enabling or disabling auto updates checks that the state directory is writable before changing systemd. It invokes the bounded host-runtime permission repair when needed and stops with an actionable error if ownership cannot be corrected. |
