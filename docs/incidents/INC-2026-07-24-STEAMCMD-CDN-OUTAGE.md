...
| **Report Standard** | Structured per NIST SP 800-61r2 incident lifecycle phases, adapted for an availability/infrastructure incident; formatted as an operator-level NOC/SOC-style postmortem. Revised twice on 2026-07-24 — first following technical review, then again after real upstream shipped a fix addressing two open recommendations — see Section 12 (Revision Notes) for what changed and why. |
...
These recommendations were originally checked against the current state of both this operator's fork and the real upstream project (`Red-Blink/dune-awakening-selfhost-docker`, latest published release `v1.3.64`) as of this report's previous revision date. At that time, the relevant scripts were byte-identical between this fork and upstream, and neither contained CDN-host-failure detection, backoff/jitter beyond the existing fixed-interval retry, priority/host-selection logging, or systemd-timer-path validation. **This has since changed**: real upstream published `v1.3.65` (2026-07-24, after the previous revision of this report), which includes commit `7b335c5` (`fix(updates): recover from stale timers and Steam host failures`) — a substantial, independently-authored fix that directly addresses R2 and R3 below. This fork's `main` has been synced with that release. R2 and R3 are marked **Resolved upstream** accordingly; R1, R4, R5, R6, and R7 remain open.
...
| **R2** | ~~P1 — High, but scoped narrowly~~ **Resolved upstream** | Improve the *automated* update pipeline's handling of a single unreachable top-priority content-source: e.g., detect that the selected host is unreachable and log this clearly, add real backoff/retry against the *same* directory-service response before giving up, and/or surface enough diagnostic detail (selected host, priority class) for a human to diagnose the same class of failure faster next time. **This recommendation explicitly does not extend to automating the TLS-interception technique used in this incident** (installing a CA, intercepting Steam traffic, rewriting hostnames, forwarding to a different CDN) as routine or automatic updater behavior — that technique is security-sensitive, brittle against Valve-side changes, and was a manual, fully-informed, one-time decision by an operator with full context, not something that should run unattended or without explicit human awareness each time. See R5 for the appropriate way to preserve this technique for future use. | **Status: Resolved upstream, verified 2026-07-24.** Real upstream commit `7b335c5` adds `runtime/scripts/steamcmd-signals.sh`, which pattern-matches SteamCMD logs for DNS failures and content-host failures (including the exact "failed to send manifest request" → "Connection timeout" signature observed in this incident), extracts the failing hostname/source-priority-class/interface-count, and rewrites `update.sh`'s retry loop to detect this failure class specifically, extend the retry budget from the previous fixed 3 attempts up to `DUNE_STEAMCMD_CONTENT_MAX_ATTEMPTS` (default 6), and back off with increasing delay (capped at 120s) — giving Steam's directory service more chances to return a different, healthy host on a later request. This is a legitimate, non-interception fix (no TLS proxying, no CA trust manipulation) — exactly the class of fix this recommendation asked for, implemented independently by the real upstream maintainer rather than by this fork. Verified via direct commit inspection and two passing upstream-authored tests (`tests/steamcmd-signals-test.sh`, `tests/update-systemd-timer-health-test.sh`) after merging `v1.3.65` into this fork's `main` (merge commit `41434d6`). The TLS-interception exclusion in this recommendation still stands and is unaffected by upstream's fix — it was never adopted as automated behavior. |
| **R3** | ~~P1 — High~~ **Resolved upstream** | Repair the automated hourly self-update systemd timer against the current, correct repository path, and add alerting if the underlying service fails N consecutive times — closing the Section 6.3 gap. Also consider a startup/periodic self-check that detects when a systemd unit's configured working directory or executable path no longer exists on disk (the actual failure mode in this incident), independent of the update-specific timer, since this class of failure (stale path after a repo relocation) could recur for other scheduled jobs. | **Status: Resolved upstream, verified 2026-07-24.** The same upstream commit `7b335c5` adds `show_auto_timer_status()` to `update.sh`, which directly checks the live systemd unit's actual `WorkingDirectory` and `ExecStart` properties against the current checkout path and emits an explicit `WARN` line if they diverge (the exact failure mode that caused this incident's contributing factor, Section 3.4) or if the timer is active while auto-updates are disabled in local state. Verified via direct commit inspection and the passing `tests/update-systemd-timer-health-test.sh`, which exercises both the healthy-timer and stale-WorkingDirectory cases against a mocked `systemctl`. This still leaves the alerting half of the original recommendation (notifying an operator after N consecutive failures, rather than only surfacing a warning on manual `dune update auto status`) as a smaller, separately-open gap — see the new R8 below. |
| **R4** | **P2 — Medium** | Investigate a genuine external check of player-facing discoverability, rather than relying on `dunedocker.app`'s heartbeat (which, per the Section 5.4 correction, only confirms our own service is reporting to itself, not that Funcom is listing it). This may require researching whether Funcom exposes any public server-list or status API suitable for this purpose; if none exists, this recommendation should be revised to rely on faster player-facing feedback channels instead, rather than a synthetic check that cannot actually verify the thing it claims to verify. | **Status: Open.** |
| **R5** | **P2 — Medium** | Document the TLS-interception CDN-bypass technique (Section 5.6, attempt #7; Section 5.7) as an **expert-only, manual emergency runbook procedure** — not as an automated fallback (see R2). The runbook should explicitly state: what TLS interception is and is not doing (Section 5.7), the conditions under which it is an acceptable tradeoff (public, non-sensitive content only), step-by-step setup and — critically — verified cleanup instructions (removing the generated CA trust, the proxy process, and any hosts-file entries), and an explicit warning that this is not standard operating procedure. | **Status: Open.** Now lower urgency than originally assessed, since R2's upstream fix substantially reduces how often this class of failure would require any manual intervention at all — but still worth documenting for the residual cases upstream's retry budget doesn't cover (e.g. a content host that stays unreachable well beyond 6 attempts). |
| **R6** | **P3 — Low** | Consider reporting the specific unreachable CDN hostname and the observed all-or-nothing single-source fallback behavior to Valve/Steamworks, as a data point for their own review — but frame this to Valve as an observation with the operator's own network cell and timestamp, not as an asserted defect, since this investigation could not confirm the underlying cause of either observation (Section 3.3). | **Status: Open.** |
| **R7** | **P3 — Low** | Formalize the player communication process for outages of uncertain root cause — the interim status message used in this incident (Section 9) was effective but improvised, and correctly avoided overclaiming certainty about the cause at the time it was posted. A pre-approved template that structurally separates "what we know" from "what we suspect" would help maintain that discipline under pressure in future incidents. | **Status: Open.** |
| **R8** | **P2 — Medium (new, added on this revision)** | Add operator-facing alerting (not just a `WARN` line on manual `dune update auto status` invocation) when the automated update timer's health check (R3, now resolved) detects a stale `WorkingDirectory`/`ExecStart` or N consecutive failures — e.g. via the project's existing Discord/notification integration. R3's upstream fix makes the underlying stale-path condition *detectable*; it does not make it *proactively surfaced* to an operator who isn't actively running status commands. This is a smaller, separately-scoped gap that R3's resolution exposed rather than closed. | **Status: Open.** |
...
| Recommendations checked against current upstream state | ✅ Yes — Section 7 (R2/R3 confirmed resolved by real upstream `v1.3.65`, commit `7b335c5`, verified 2026-07-24; R1/R4-R7 remain open, R8 added) |
...
9. **(2026-07-24, second revision)** Real upstream (`Red-Blink/dune-awakening-selfhost-docker`) published `v1.3.65`, including commit `7b335c5` (`fix(updates): recover from stale timers and Steam host failures`) — a substantial, independently-authored fix directly addressing R2 (Steam content-host retry/backoff) and R3 (stale systemd timer path detection). Verified by direct commit inspection (not assumed from the commit message alone) and by running upstream's own new tests (`tests/steamcmd-signals-test.sh`, `tests/update-systemd-timer-health-test.sh`), both passing, after merging `v1.3.65` into this fork's `main` (merge commit `41434d6`). R2 and R3 marked **Resolved upstream** in Section 7; added a new R8 covering a smaller, separately-scoped gap R3's fix exposed (detection without proactive alerting).
