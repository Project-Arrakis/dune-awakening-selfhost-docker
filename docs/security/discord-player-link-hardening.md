...
- **Implemented:**
  1. `linkPlayerProvider()` now calls `getLinkedPlayer(db, discordUserId)` immediately after resolving the requested character name, before any online-status/Funcom-ID/Steam-shortcut checks. If a link already exists for a *different* `player_controller_id`, the request is rejected with `{ ok: false, error: "..." }` (not a thrown error, matching this function's existing convention for user-facing rejections like "must be online"/"no active Funcom identity"). Re-requesting a link for the *same* already-linked character is allowed to proceed unchanged — an idempotent re-link (e.g. a player re-running the command after losing track of whether it worked) is not treated as a conflict.
...
- **Not implemented (explicitly deferred to phase two, tracked separately, not part of this pass):**
  ...
  - The bot-side (`arrakis-control-panel`) requires no code change for the whisper-flow rejection (`linkPlayerProvider()`'s `{ ok: false, error }` return) -- confirmed by direct review that `formatLinkEmbed()` already displays `payload.error` verbatim in a clean "🔗 Link Failed" embed with no changes needed. The Steam-link/multi-account rejection (`linkAdditionalAccount()`'s thrown 409) is displayed correctly but less cleanly: confirmed by direct review that the bot's generic `formatError()` prefixes it with "Adapter request failed: " rather than a dedicated embed, since Steam-link errors are surfaced via HTTP-error handling, not a `{ok: false}` payload. The message itself is still fully readable; only the presentation differs. Improving this is a real, minor, optional bot-side polish item, not a blocker for this fix.
