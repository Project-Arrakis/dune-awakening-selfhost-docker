...
- **2026-07-25 update:** confirmed this exposure is still live on `main` today (no `DUNE_COMMAND_AUTH_TOKEN` override set on this fork's production deployment) and that `RMQ_GAME_PORT` is bound on all interfaces (not `127.0.0.1`), not just the loopback-only admin RMQ instance — making this a real, network-reachable exposure, not merely a source-code weakness. See `docs/security/command-auth-token-vulnerability-and-failed-remediation.md` for the dedicated writeup, including confirmation this exact issue was found a third time (as an uncommitted, half-applied diff in a stale local worktree) and deliberately not re-applied, plus 4 candidate remediation paths for future work.
...
### FINDING-LINK-8: Phase-one 1:1 linking constraint — multi-account capability deliberately gated off (Design Decision, not a vulnerability) — IMPLEMENTED

- **Location:** `console/api/src/integrations/discord/linkProvider.js` (`linkPlayerProvider`), `console/api/src/duneDb.js` (`linkAdditionalAccount`).
- **Context:** FINDING-LINK-6/7 built and shipped a genuine, tested, working multi-account linking system — a Discord user can hold several linked characters, with one marked default, switchable via `setDefaultAccountProvider()`, and Steam-connections-based linking (FINDING-LINK-7) writes into the same multi-account table exclusively. All of this remains real, live, and fully functional at the data layer. However, no Discord slash command was ever built to actually let a player *use* more than one linked character (`/dune player enable`/`disable`/`default` — the commands that would expose "which of your characters is active in this guild" — call a `guild-character-grants` feature that has never existed on Core at all, confirmed via direct grep; this is a separate, real, tracked gap, not part of this finding). In practice, this meant a player could accumulate 2+ linked characters via `/dune player link`/Steam-link with no way to ever choose which one any of `whoami`/`inventory`/`storage`/`find` actually operate on beyond whichever happened to be `is_default` — a confusing, effectively broken experience for any player who ever linked a second character, discovered via direct operator review of the real command surface (2026-07-27), not a live incident.
- **Decision (explicit operator direction, 2026-07-27):** rather than rushing `enable`/`disable`/`default` into existence to make multi-account usable, enforce a strict 1:1 relationship for phase one — **one Discord user may link exactly one character, globally, until they explicitly unlink it.** A second `/dune player link` (or Steam-link) attempt for a genuinely different character is rejected with a clear error, rather than either silently succeeding (the multi-account system's original behavior) or silently overwriting the existing link (the legacy single-link table's original `on conflict (discord_user_id) do update` behavior — see FINDING-LINK-6's "Original state" note). Multi-account capability itself — the tables, the routes, `unlink`/`list`/`set-default`, Steam-link — is **not removed or disabled**; it is intentionally kept intact, tested, and ready for a real phase two once `guild-character-grants` (or an equivalent mechanism for choosing an active character) actually exists and is exposed to players.
- **Implemented:**
  1. `linkPlayerProvider()` now calls `getLinkedPlayer(db, discordUserId)` immediately after resolving the requested character name, before any online-status/Funcom-ID/Steam-shortcut checks. If a link already exists for a *different* `player_controller_id`, the request is rejected with `{ ok: false, error: "..." }` (not a thrown error, matching this function's existing convention for user-facing rejections like "must be online"/"no active Funcom identity").
  2. `linkAdditionalAccount()` (the multi-account/Steam-link entry point) gained the equivalent check, but must look in **both** tables — a user with an existing legacy single-link for a different character must be rejected here too, and vice versa is already covered by `linkPlayerProvider()`'s check above. Implemented as two new lookups (`discord_player_links` first, `discord_account_links` second) inside the existing transaction, alongside (not replacing) FINDING-LINK-6's own cross-table ownership check (`otherTableLinkConflict()`, which answers "does someone ELSE own this character" — a different question from this fix's "does THIS user already own a DIFFERENT character"). On conflict, throws a new `user_already_has_a_character` error (409), constructed via a fresh `getLinkedPlayer()` call so the message can name the character the user is already linked to.
  3. Error copy is intentionally in-lore rather than a generic rejection, per explicit operator direction: *"Your voice already answers to \<character\> in the eyes of the Landsraad. A soul may not walk two paths in the desert -- use /dune player unlink before you may bind yourself to \<new character\>."* (whisper-flow variant, names both characters) and *"...before you may bind yourself to a new name."* (multi-account/Steam variant, names only the existing character, since the second one isn't always known by name at that point in the flow).
- **Follow-up fix, found via a real live report the same session (2026-07-27):** re-requesting a link for the *same* already-linked character was originally left to "proceed unchanged" (fall through to the existing online/Funcom-ID/whisper/Steam-shortcut logic) on the theory that a harmless idempotent re-link shouldn't be treated as a conflict. In practice this meant a player re-clicking "Link via Steam" for a character they already had linked would be sent through a full external Discord OAuth round-trip, only to have `linkAdditionalAccount()` reject the resulting call at the very end with the older, generic `already_linked_to_this_account` message (not even this finding's own in-lore gate message, since that check only fires for a genuinely *different* character) — after the round-trip had already happened for nothing. Real operator question that surfaced this: *"why send the player a link button when we already can determine that they are already linked?"* There was no good answer. Fixed by adding an explicit same-character short-circuit **before** the `hasSteam`/online/Funcom-ID checks in all three real entry points (`linkPlayerProvider()`, `linkAccountProvider()`, `linkAccountViaSteamProvider()`), returning `{ ok: true, alreadyLinked: true, message: "..." }` immediately — no whisper is sent, no Steam-link button is offered, no external round-trip happens, and no pending verification code is created for a request that was always going to be a no-op. `linkAccountViaSteamProvider()`'s check is kept even though `linkPlayerProvider()` should no longer offer this button for that exact scenario in normal use, since this route is independently callable (`PLAYERS_ACCOUNTS_LINK_STEAM`) and not solely reachable through `linkPlayerProvider()`'s own flow.
- **Verification:** Every existing test exercising 2-character-per-user scenarios was reviewed individually, not blanket-updated — three categories emerged: (a) tests whose entire subject *was* successfully acquiring a second character (e.g. "first linked account becomes default automatically; second does not") were rewritten to assert the new rejection instead; (b) tests whose real subject was still-valid, unaffected behavior further down the lifecycle (unlink's default-promotion, `setDefaultAccountProvider`'s switching, `discordPlayerUnlink`'s cross-table read) had their 2-account setup changed from calling the now-gated `linkAdditionalAccount()` twice to seeding the second account directly into mock state, preserving coverage of that real, unaffected behavior; (c) several draft test assertions written during this fix were caught and corrected mid-work after they failed for reasons that revealed the initial assertion itself was wrong (verifying the *same* character twice originally hit the pre-existing `already_linked_to_this_account` conflict rather than this finding's new gate, confirmed by intentionally running the test and reading the real failure — this was itself the exact gap the follow-up fix above closes, discovered through the test-writing process, not just the live report). `console/api` 780 → 788 tests, all passing.
- **Not implemented (explicitly deferred to phase two, tracked separately, not part of this pass):**
  - `/dune player enable`/`disable`/`default` remain non-functional (call the nonexistent `guild-character-grants` feature) — building this (or an equivalent "choose your active character" mechanism) is the actual prerequisite for ever relaxing the phase-one 1:1 gate back to genuine multi-account usability.
  - No migration or cleanup was needed or performed: confirmed via direct review that no player has more than one linked character in the only known live deployment at the time of this change, so the new gate has no pre-existing state to reconcile against.
  - The bot-side (`arrakis-control-panel`) requires no code change for either the whisper-flow rejection or the new same-character `alreadyLinked: true` success response — confirmed by direct review of BOTH real consumers. `formatLinkEmbed()` (the whisper-flow's own formatter) displays *some* confirmation for any `{ ok: true }` payload, and its existing failure path already displays `payload.error` verbatim in a clean "🔗 Link Failed" embed. `steamLinkServer.js`'s Steam-link web callback (a separate code path, not `formatLinkEmbed()`) checks `linkResult?.matched` -- `alreadyLinked: true` responses set `matched: true`, so this correctly falls into the existing success branch (`findLinkedCharacterName()` reads the real character name from the `accounts` array this response already includes) rather than the "Unable to Link" error page a real operator actually saw before this fix, confirming the exact reported symptom no longer occurs. Whether either success path's copy reads sensibly for an `alreadyLinked: true` response specifically ("Linked as X" vs. a more explicit "You're already linked to X") was not evaluated as part of this pass — a real, minor, optional bot-side polish item, not a blocker. The Steam-link/multi-account rejection for a genuinely *different* second character (`linkAdditionalAccount()`'s thrown 409) is displayed correctly but less cleanly: confirmed by direct review that the bot's generic `formatError()` prefixes it with "Adapter request failed: " rather than a dedicated embed, since Steam-link errors are surfaced via HTTP-error handling, not a `{ok: false}` payload. The message itself is still fully readable; only the presentation differs.
