...
**Every mutating write that could increase a tier's permissions is also rejected if it would grant that tier something the ACTING session doesn't already, independently hold** (`editPolicy`, `rollbackPolicy`, `setTierInline`, `attachPolicy`, and the legacy `setPolicies` route). This closes a real privilege-escalation path found during this design's Layer 3 audit: without this check, a tier holding only `settings:write` could mint a brand-new unconditional-Allow policy and attach it to its own tier, reaching full owner-equivalent access in two API calls. This mirrors AWS IAM's own permissions-boundary concept — `iam:CreatePolicy`/`iam:AttachRolePolicy` alone never lets a principal grant a permission it doesn't already have — and, deliberately, is **not** a hardcoded "only owner may manage IAM" identity check; a custom tier that genuinely already holds broad permissions (e.g. one an owner has already granted `Allow:"*"`) can still grant that same breadth to other tiers, exactly as the owner tier itself always could. Operations that can only ever *remove* a grant (`detachPolicy`, `deletePolicy`, `deletePolicyVersion`) are not subject to this check, since they cannot amplify anything.
