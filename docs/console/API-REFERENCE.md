...
| POST | `/api/setup/discord-finalize` | Complete the guided setup from an owner session: admin password again (fresh proof), a server the captured identity **owns**, and the role mapping; writes the config and reports `restartRequired` | `adminPassword`, `guildId`, `adminRoleIds`, `moderatorRoleIds`, `playerRoleIds`, `requireMfa` |
