...
| T35 | | | |
| T36 | | | |
| T37 | Partial | Ran live on dune-dev, 2026-09-03, against commit `fb705caa` — before the in-process cutoff flags (`discordOAuthSoftDisabledInProcess`/`discordOAuthForgottenInProcess`) existed. `discordOAuthDisabled`/`discordOAuthConfigured` flipped correctly at each step and the sign-in screen matched exactly; that run found and led to fixing a real bug (`DISCORD_OAUTH_DISABLED` written to `.env` but never reaching the container — missing `docker-compose.web.yml` passthrough). **What it did NOT test: the actual security property this PR now provides** — the disable/forget cutoff taking effect immediately, in-process, independent of whether the restart completes. That mechanism (and the Enable-cannot-reverse-a-Forget fix) was added afterward, in response to the Layer 2/3 audits, and is currently automated-test-covered only (`discordOAuthDisable.integration.test.js`'s three immediate-cutoff tests) — not yet re-run live against this exact head. | Owner: re-run T37 live against current head before this PR leaves draft; the automated coverage is real but a live pass on the actual security fix hasn't happened yet. |
