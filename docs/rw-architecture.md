...
**Internal dispatch transport ("Hop B"):** `write/execute` never reimplements a mutation — it dispatches to the exact same, unmodified route handler the web console itself calls, over an internal loopback. That loopback is a Unix domain socket (filesystem-permission-gated, mode `0700`, refuses to start entirely if Core is running as root, since root bypasses Unix DAC and the permission gate would be meaningless), not a second TCP listener bound to `127.0.0.1`. A TCP loopback was considered and rejected: it relies on network-namespace isolation holding identically across every deployment topology this project supports (bare-metal, various container/orchestration setups), which is exactly the kind of assumption that silently breaks for some subset of operators. A Unix socket's reachability is a filesystem permission, not a network address, so it doesn't carry that same topology dependency. A third option — restructuring Core's own route dispatch so it could be called as a plain in-process function instead of over any loopback at all — was also considered and rejected as a much larger, riskier change to how every existing route is wired, for a benefit (skipping the loopback) that a Unix socket already delivers at a fraction of the cost.
