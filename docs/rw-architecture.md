...
unguarded by any regression test (QA). Filed as #763-776. Full findings
and STRIDE reports for all five completed rounds: comments on #215. This
section of the doc, and every section below it, reflects the design after
remediating all CRITICAL/HIGH findings from rounds 2 through 6. A
**round-7** Eight-Hat Layer 1 audit against this further-corrected version
is still required before Layer 2 begins, per the exit gate stated above —
do not assume this pattern has stopped just because this round's fixes
look thorough; every prior round looked thorough too, and round 6 found
more findings than any round before it.
...
**Corrected after round-6 audit (was MEDIUM, batch #776, UI/UX hat): the dual-confirmation gate had no described end-to-end mechanism anywhere in this document — a real "no working path for this state" gap, not a wording nit.** The requirement was stated once, in the sentence above, with no design for: what embed/notification the second administrator sees, how they discover the pending request, whether there's a timeout if no second admin responds, or whether the original requester can serve as their own second confirmer (they must not be able to). **Design:** the initial `stop` confirmation posts a second, separate embed to the same channel (not a DM — visibility for any admin who happens to be present is the point), tagging the `admin`/`owner`-tier Discord role(s) and showing who initiated the request; any *different* eligible admin clicking its own Confirm button satisfies the gate (the original requester's user ID is explicitly excluded from being able to click it, enforced server-side via the actor signature's `userId`, not just hidden client-side). This second embed expires on the same 30s countdown as the primary confirmation — if no second admin confirms in time, the whole `stop` request is cancelled and the bot posts a clear "not enough confirmations, action cancelled" message, never silently proceeding on a single confirmer. Layer 2 must add this to Section 6's test plan: a `stop` request where the same user attempts both confirmations must be rejected; a `stop` request with two distinct eligible admins must succeed; a `stop` request where the second confirmation never arrives must cancel cleanly within the 30s window.
...
- **None**: start, enable, disable, scan, warn
...
| Idempotency cache persistence | **DESIGNED** (Section 3.8 — persisted via `jsonStore.js`'s atomic writer + a per-key `Map<idempotencyKey, Promise>` lock with settled-entry cleanup tied to the real mutation's own settlement (not a timeout wrapper), plus `runExclusive()`-style serialization between the persisted cache's own writes and its periodic pruning sweep, after round-4 found round-3's `restartQueue.js`-convention fix had a race, round-5 found round-4's own global-queue fix was itself a new DoS vector, then found the per-key fix's own Map had no cleanup, then round-6 found the cleanup fix itself could delete a still-in-flight entry and that the pruning sweep was unsynchronized with `write/execute`'s own cache write) — not built | #736, #746, #752, #758, #769, #770 |
