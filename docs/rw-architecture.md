...
**Date:** 2026-08-08 (revised 2026-09-09, corrected after round-2, round-3, round-4, round-5, round-6, and round-7 audits, plus a mechanical cross-reference pass after round 6, same day)
...
reports for all completed rounds, plus #777: comments on #215. This
section of the doc, and every section below it, reflects the design after
remediating all CRITICAL/HIGH findings from rounds 2 through 7 and #777.
A **round-8** Eight-Hat Layer 1 audit against this further-corrected
version is still required before Layer 2 begins, per the exit gate stated
above — do not assume this pattern has stopped just because this round's
fixes look thorough; every prior round looked thorough too, and round 7
found the highest CRITICAL count of any round yet.
...
**Corrected after round-6 audit (was MEDIUM, batch #776, UI/UX hat): the dual-confirmation gate had no described end-to-end mechanism anywhere in this document.** Round 6's own first-pass design was itself substantially redesigned after round 7 found real problems from five different hats (UI/UX, Security, Architect, QA — #782/#785/#786/#787): no fallback for single-admin deployments (plausibly the majority profile for a self-hosted fork — the gate as first designed made `stop` permanently unusable for a solo operator); a `userId`-based exclusion that one operator with two Discord accounts can defeat; a reused 30s self-cancel timer that's unrealistic for a genuinely different human to notice and respond to; no reconciliation with the 60s nonce TTL; and an enforcement mechanism that never said which server (Core, via the existing nonce/signature machinery, or the bot, unverifiably) actually checks the exclusion, or whether the second confirmer's role is checked with the same signed-freshness rigor (`roleSnapshotAt`) the rest of this design insists on for the primary actor.

**Redesign (round 7):**

1. **Single-admin/owner fallback, closing #785's usability gap.** At `write/preview` time, the bot checks its own cached role membership for how many distinct Discord users currently hold `admin` or `owner` tier on that guild. If exactly one (the requester themself), the dual-confirmation requirement is waived — `stop` proceeds through the normal single-actor flow (typed `"STOP"` phrase, 30s cancellable countdown, same as `restart`). This is a deliberate, documented design choice, not an oversight: demanding a second person who structurally doesn't exist would make `stop` permanently unusable for the plausible majority of self-hosted deployments. **Residual risk, stated explicitly (matching Section 3.4's pattern):** for a solo-admin server, `stop` is exactly as unilaterally executable as every other owner-tier action already is — this gate adds protection only where a second real human is actually available to provide it.

2. **Enforcement mechanism, closing #787's ambiguity.** The second confirmation is its own, independent `write/preview`→`write/execute` round-trip, not a reuse of the primary actor's nonce. When the primary admin's typed-phrase confirmation succeeds, Core creates a *pending stop request* (a short-lived server-side record: the initiating `userId`, guild, timestamp) instead of executing immediately, and the bot posts the second embed. When a different eligible admin clicks its Confirm button, the bot runs a full second `write/preview`→`write/execute` cycle for that admin — going through `verifyActorSignature()`, `requireDiscordCapability()`, and a **fresh** `roleSnapshotAt` check exactly like any other actor, closing the gap where the second confirmer's authorization was checked with less rigor than the first. Core's `stop` handler requires both a valid second-confirmation record referencing the same pending-stop-request ID *and* that the second actor's `userId` differs from the pending request's initiating `userId` (compared server-side, not client-side) before it actually executes. This also cleanly resolves #786's nonce-TTL-reconciliation gap: the second confirmation has its own independent 60s nonce TTL, not a shared clock with the primary.

3. **Timing budget, closing #786's unrealistic-window gap.** The pending-stop-request record (not the second confirmer's own 60s nonce, which starts fresh once they click) has its own, much longer window — **5 minutes** — to actually be acted on, explicitly distinct from the primary's 30s self-cancel timer: soliciting a different human's attention is a fundamentally different problem than giving the same person a chance to abort, and reusing the shorter timer for both was the original design's mistake.

4. **`userId`-exclusion residual risk, closing the rest of #785.** A `userId` check cannot distinguish two genuinely different humans from one person operating two Discord accounts that both hold `admin`/`owner` — this is a Discord-identity-model limitation no technical control here can close. **Stated explicitly as a residual risk**, matching Section 3.4's pattern: this gate assumes role grants aren't held by one individual under two identities; that assumption is a guild-administration/GRC responsibility (who gets the `admin`/`owner` role, and on how many accounts), not something this control can verify.

5. **Second-embed content, closing the MEDIUM info-gap (batch #791, UI/UX hat).** The second embed shows the same live player count the primary confirmation shows, plus an optional reason string (`stop`'s command signature gains an optional `[reason]` parameter, shown on both embeds) — a second approver needs the same information the first one had to make an informed decision, not just a name and a button.

6. **Visual differentiation, closing the MEDIUM UI ambiguity (batch #791, UI/UX hat).** The second embed uses a distinct title and color (e.g. "🔒 SECOND CONFIRMATION REQUIRED") so it's never confused with the primary confirmation's own embed in the same channel.

7. **Excluded-click feedback, closing the LOW gap (batch #791, UI/UX hat).** If the original requester clicks the second embed's own Confirm button, the bot responds with a visible (ephemeral, only shown to that clicker) message — "You already confirmed this request; a different admin must provide the second confirmation" — never a silent no-op.

8. **Cancellation message, closing the LOW gap (batch #791, UI/UX hat).** On a 5-minute window lapse with no second confirmer, the bot posts "not enough confirmations, action cancelled" (unchanged from round 6) and additionally names the console UI as a direct alternative for an operator who needs to stop the server outside this gate.

Layer 2 must add all of the following to Section 6's test plan (was HIGH #782, QA hat — round 6's identical commitment was never followed through): single-admin fallback correctly waives the gate when exactly one eligible admin/owner exists; the gate correctly activates when 2+ exist; a `stop` request where the same user attempts both confirmations is rejected (server-side, `userId` comparison); a `stop` request with two distinct eligible admins succeeds, with the second confirmer's own `verifyActorSignature()`/`requireDiscordCapability()`/fresh-`roleSnapshotAt` checks independently exercised, not assumed; a `stop` request where the second confirmation never arrives cancels cleanly within the 5-minute window; the second embed's content (player count, reason) and visual distinction from the primary embed.
...
| `grant-all` | write/execute → `POST /api/care-package/grant-eligible` | `carepackage:grant-all` | **owner** | Yes (shows eligible count) |
...
- *(new, Core)* **`resolveWriteBridgePrincipal()` unit contract, added after round-7 audit (was HIGH #783, QA hat):** call the function directly (not through `requestHandler`/`handleApi`) across all four combinations of correct/wrong token × correct/wrong path, asserting its raw return value (principal object vs. `null`); separately assert that `handleApi`/`auth.requireAuth`, given a pre-resolved principal via `opts`, do not independently re-invoke the token comparison (a call-count/spy assertion on the underlying comparison function).
- *(new, Core)* **`stop` dual-confirmation gate, added after round-7 audit (was HIGH #782, QA hat — round 6's identical commitment in Section 2's Safety note was never followed through):** single-admin fallback correctly waives the gate when exactly one eligible admin/owner exists on the guild; the gate correctly activates when 2+ exist; a `stop` request where the same user attempts both confirmations is rejected via a server-side `userId` comparison against the pending-stop-request record, not a client-side check; a `stop` request with two distinct eligible admins succeeds, with the second confirmer's own `verifyActorSignature()`, `requireDiscordCapability()`, and a fresh `roleSnapshotAt` check all independently exercised (not assumed inherited from the primary actor's checks); a `stop` request where the second confirmation never arrives cancels cleanly within the 5-minute pending-request window; the second embed's content includes the live player count and any stated reason, and is visually distinct from the primary confirmation embed.
