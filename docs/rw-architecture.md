...
**Corrected after round-4 audit (was CRITICAL #750, found independently by Architect + Network hats, Network hat's finding empirically reproduced): `requestHandler`'s real, pre-existing `ADMIN_ALLOWED_IPS` gate must exempt Unix-socket requests, or the write bridge is permanently non-functional for any operator using that config.** `server.js`'s real dispatcher checks `config.allowedIps` *before* any routing/auth logic runs, on every request — `req.socket.remoteAddress` is `undefined` for a Unix-socket connection (empirically confirmed), which normalizes to `""`, which can never match a configured allowlist entry, so every write-bridge request would be unconditionally 403'd for any operator with `ADMIN_ALLOWED_IPS` set (the officially-documented, code-enforced compensating control for `ADMIN_AUTH_DISABLED=1` on a non-loopback bind — exactly the operators running the most security-conscious supported configuration). **Fix:** the `ADMIN_ALLOWED_IPS` check must exempt any request where `opts.viaWriteBridgeSocket === true` — reachability for that listener is already filesystem-permission-gated (below), so re-applying an IP-based check on top of it is redundant and, as shown, actively broken.
...
**Corrected after round-4 audit (was MEDIUM, batch #755, DBA hat): an unreadable/corrupt cache file must fail closed, not fail open to empty.** `secondFactorStore.js`'s own corruption handling (log a warning, then proceed as if the store were empty) is the right behavior for *that* store, but wrong for this one — "empty" specifically means "no evidence this action already executed," which is precisely the condition that lets a retried `write/execute` silently re-run a non-idempotent mutation. **Fixed:** if the idempotency-cache file exists but fails to parse, `write/execute` rejects the in-flight request with a `503`-class error ("idempotency store unavailable, cannot safely verify this hasn't already run") rather than silently treating the key space as empty, and logs at a severity that would actually surface to an operator monitoring the process, not just a debug-level warning.
...
- *(new, Core)* **Stale-role rejection (#734) and freshness enforcement (#744/#749/#754):** an actor whose role is revoked between `write/preview` and `write/execute` is rejected; three-case table for `roleSnapshotAt` (round-4 audit, QA + DBA hats, batch #755 and #754 — a prior revision only stated the negative case, an inverted-tautology risk): a fresh `roleSnapshotAt` is **accepted**; a stale one (`Math.abs(now - roleSnapshotAt) > maxRoleAgeSeconds`) is **rejected**; a **future-dated** `roleSnapshotAt` is also rejected (not just old ones — the bounded-both-directions formula, not the original one-sided formula); the boundary at exactly `maxRoleAgeSeconds` is explicitly pinned to one behavior. Also confirm `write/preview`/`write/execute` verify this field via `WRITE_BRIDGE_SIGNED_ACTOR_FIELDS` (3.8), and that an old-format actor payload lacking `roleSnapshotAt` entirely still verifies correctly against `broadcast`/`players/link` (regression test for #749 — the shared `SIGNED_ACTOR_FIELDS` array must remain untouched).
...
- *(new, Core)* **Socket restart safety (#753):** start Core, leave a stale file at the socket path (simulating an unclean prior shutdown), restart, and confirm the write-bridge subsystem comes up successfully (stale file removed, no `EADDRINUSE`) rather than crashing the whole process.
- *(new, Core)* **Socket permission enforcement (#755):** assert the created socket file's mode is exactly `0700` after boot.
