...
import { DISCORD_ROLE_TIERS, DISCORD_CAPABILITIES } from "../src/integrations/discord/policy.js";
...
// (group, subcommand) pair) must still produce exactly the 4 documented
// pairs, not more or fewer -- catches both a regression (a 5th route
// accidentally merged into an existing pair) and someone "fixing" the fan
// out by incorrectly re-splitting it back into duplicates. "player.unlink"
// added 2026-08-20 (reconciling PR #182): PLAYERS_UNLINK/PLAYERS_ACCOUNTS_
// UNLINK, verified as a real dispatch in the bot's commands.js, not just an
// inferred one -- see commandCatalog.js's own comment on this pair.
...
test("routes with routeHasNoCurrentBotCaller: true are exactly the known, documented exceptions (#342)", () => {
  const catalog = buildCommandCatalog();
  const unwired = flattenRoutes(catalog)
    .filter((entry) => entry.routeHasNoCurrentBotCaller)
