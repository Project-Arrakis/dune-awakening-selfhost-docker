...
test("42 Upgrade: explicit host IDs survive .env and stale root IDs normalize to a non-root owner or caller", () => {
  ...
  assert.match(repair, /TARGET_UID="\$\{REQUESTED_HOST_UID:-\$\{DUNE_HOST_UID:-\$OWNER_UID\}\}"/);
  assert.match(repair, /TARGET_GID="\$\{REQUESTED_HOST_GID:-\$\{DUNE_HOST_GID:-\$OWNER_GID\}\}"/);
  assert.match(repair, /"\$TARGET_UID" = "0".*"\$OWNER_UID" != "0"/);
  assert.match(repair, /TARGET_UID="\$OWNER_UID"/);
  assert.match(repair, /"\$TARGET_UID" = "0".*"\$CALLER_UID" != "0"/);
