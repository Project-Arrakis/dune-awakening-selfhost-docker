...
// Upstream review finding (PR #157): discoverDbConfig() previously
// implemented its own precedence (DUNE_DB_PORT || PGPORT ||
// resolvePorts().postgres), which disagreed with resolvePorts()'s own
// precedence (POSTGRES_PORT || DUNE_DB_PORT || PGPORT) whenever an
// operator had more than one of these set to different values --
// status/preflight (which reads resolvePorts() directly) could then
// disagree with the actual database connection (which read this
// function), a real, silent split-brain misconfiguration.
// discoverDbConfig() must now always delegate to resolvePorts() for
// this field so there is exactly one place this precedence logic can
// ever drift from itself.
