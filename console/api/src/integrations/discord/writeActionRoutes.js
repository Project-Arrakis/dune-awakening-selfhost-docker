...
const RAW_WRITE_ACTION_ROUTES = {
  ...
  // carepackage.*). Combined with server.stop's own dual-confirmation
  // removal (#404/#1029), this meant one owner-tier actor could stop the
  // server with less friction than banning a player. The bot's own
  // preview/confirm UX reads this field from write/preview's response and
  // requires the Discord admin to type it before the confirm control
  // activates -- see the module header comment for why this is a UX
  // safeguard, not an independent security boundary.
  ...
  "carepackage.grant-all": { method: "POST", path: () => "/api/care-package/grant-eligible", confirmPhrase: "GRANT CARE PACKAGE TO ELIGIBLE PLAYERS", policyAction: "carepackage:grant-all", auditAction: "care-package.grant-eligible" },
