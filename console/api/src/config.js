...
export function resolvePorts(env = process.env, repoRoot = process.cwd()) {
  ...
  // base port itself (1-65535), but every Player/Game and IGW base is
...
function readEnginePortsFromProfile(repoRoot) {
  ...
  // entirely (a stripped-down or hand-edited profile file), this
  // previously fell back to scanning the WHOLE file for a
  // Port=/IGWPort= match -- which could silently pick up an unrelated
  // key from a different section (e.g. [Engine:ConsoleVariables]'s own
  // Port= override, which means something entirely different) and
  // report it as the real game port. The real Python tool
  // (usersettings.py's profile_get_key()) only ever looks inside the
  // named section; if the section doesn't exist, it returns nothing for
  // that key, not a value from elsewhere in the file. Match that
  // exactly: no section match -> no port match, fall through to the
  // legacy usersettings.json / stock fallback chain.
