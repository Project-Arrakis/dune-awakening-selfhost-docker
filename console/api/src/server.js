...
async function handleApi(req, res) {
  ...
  if (path === "/api/auth/state") {
    ...
    //
    // discordSetupPendingRestart is live, not baked into publicConfig: true
    // when setup wrote a config the running process has not loaded yet (it
    // reads .env only at boot). Lets the sign-in page show "configured --
    // restart pending" rather than re-offer setup, which is the loop an
    // operator hits between finalize and restart.
