...
const navGroups: { title: string; items: { tab: Tab; icon: React.ReactNode; requiredAction?: string }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { tab: "Vehicles", icon: <Car size={18} />, requiredAction: Action.VEHICLES_READ },
      { tab: "Exchange", icon: <Store size={18} />, requiredAction: Action.EXCHANGE_READ },
...
export function App() {
  ...
  // Tier 3 (RFC docs/rfc-console-auth.md §2.3/§4) password+TOTP login state.
  ...
  useEffect(() => {
    const handleSessionExpired = () => {
      ...
      // Tier 3 (#407): without this, a session expiry that lands while
      // totpRequired/setupMode is set would leave the login screen stuck --
      // no password field (totpRequired keeps rendering the code/recovery
      // field) or a dead-end setup screen with no way back to plain login.
  ...
  // Tier 3 login (RFC §2.3/§4): uses loginRequest, not post()/api(), because
  // every status this route returns (200 authenticated/enrollmentRequired/
  // resetupRequired, 401 wrong-password/totpRequired/recoveryFailed, 429
  // rate-limited) is a real outcome to branch on -- there is no session yet
  // at login time, so the shared session-expiry-on-401 handling used
  // elsewhere in the app would misrepresent all of these.
  ...
  // After enrollment/re-setup completes, the server invalidates that session
  // and requires a fresh, normal password+TOTP login (RFC §4) -- return to
  // the plain login form rather than assuming success.
  ...
  return (
    <div className="app-shell">
      ...
      <main className={!redeploySetupOpen && tab === "Home" ? "home-main" : undefined}>
        ...
        {!redeploySetupOpen && tab === "Storage" && <LazyTabBoundary label="Loading Storage"><StoragePanel onError={setError} confirmAction={confirmDialog} formatMutationResult={formatMutationResult} waitForTask={waitForTaskSilently} /></LazyTabBoundary>}
