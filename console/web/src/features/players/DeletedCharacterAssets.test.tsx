...
describe("DeletedCharacterAssets", () => {
  ...
  it("always shows the unattributed block, including when it is empty", async () => {
    deletedCharacters.mockResolvedValue(result());
    render(<DeletedCharacterAssets />);

    await waitFor(() => expect(screen.getByText("Unattributed Orphans")).toBeTruthy());
  ...
  it("opens a base and a vehicle on the tab that owns them", async () => {
    const onOpenBase = vi.fn();
    const onOpenVehicle = vi.fn();
    deletedCharacters.mockResolvedValue(result({ characters: [kitty] }));

    render(<DeletedCharacterAssets onOpenBase={onOpenBase} onOpenVehicle={onOpenVehicle} />);

    await waitFor(() => expect(screen.getByText("kitty")).toBeTruthy());
    fireEvent.click(screen.getByText("kitty"));
    await waitFor(() => expect(screen.getAllByText("Open").length).toBe(2));

    const [baseOpen, vehicleOpen] = screen.getAllByText("Open");
    fireEvent.click(baseOpen);
    expect(onOpenBase).toHaveBeenCalledWith("3087");
    fireEvent.click(vehicleOpen);
    expect(onOpenVehicle).toHaveBeenCalledWith("3063");
  });
