...
export function CharacterAdminUI({ detail, fallback, dbPlayerId, actionPlayerId, playerName, onError, onRefresh, onClose, confirmAction, waitForTask, formatMutationResult }: { detail: Record<string, unknown> | null; fallback: Record<string, unknown>; dbPlayerId: string; actionPlayerId: string; playerName: string; onError: (text: string) => void; onRefresh: () => void; onClose: () => void; confirmAction: ConfirmAction; waitForTask: (task: Task) => Promise<Task>; formatMutationResult: (result: unknown) => string }) {
  ...
  async function playerAdmin_repairFactionReputation() {
    if (!(await confirmAction(`Synchronize faction reputation and vendor access for ${playerName}? The player must be offline.`))) return;
    await playerAdmin_runAction("repairFactionReputation", `Synchronizing ${playerName}'s faction reputation`, async () => {
      ...
      return { message: String(response.result?.message || `${playerName}'s faction reputation and vendor access were synchronized. Relog required.`) };
    }, `${playerName}'s faction reputation and vendor access were synchronized. Relog required.`, { actionType: "Repair Faction Reputation", target: playerName, amount: "1" });
  }
