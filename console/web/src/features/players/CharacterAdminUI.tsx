...
export function CharacterAdminUI({ detail, fallback, dbPlayerId, actionPlayerId, playerName, onError, onRefresh, onClose, confirmAction, waitForTask, formatMutationResult }: { detail: Record<string, unknown> | null; fallback: Record<string, unknown>; dbPlayerId: string; actionPlayerId: string; playerName: string; onError: (text: string) => void; onRefresh: () => void; onClose: () => void; confirmAction: ConfirmAction; waitForTask: (task: Task) => Promise<Task>; formatMutationResult: (result: unknown) => string }) {
  ...
  return (
    <section className="playerAdmin_container" aria-label="Player admin layout">
      ...
      <PlayerSummary detail={detail} fallback={fallback} dbPlayerId={dbPlayerId} actionPlayerId={actionPlayerId} refreshKey={playerAdmin_summaryRefreshKey} />
      ...
      {playerAdmin_activeTab === "Character" && <div className="playerAdmin_content">
        {playerAdmin_toggleBox("quick_rewards", "Quick Rewards", <div className="playerAdmin_section playerAdmin_quickRewardsSection">
          ...
          {playerAdmin_faction && playerAdmin_actionRow("faction", <span className="playerAdmin_labelWithInfo"><span>Give Faction Reputation</span><InfoTooltip id="faction-reputation-help" label="About Faction Reputation">Faction Reputation affects rank, but Ranks 1–5 also require faction story progression. Giving Reputation does not complete missions or bypass story requirements.</InfoTooltip></span>, <input type="number" min="1" max="12474" value={playerAdmin_factionAmount} onChange={(event) => playerAdmin_setFactionAmount(event.target.value)} />, "Give", () => playerAdmin_runAction("faction", `Giving ${Number(playerAdmin_factionAmount) || 0} ${playerAdmin_faction.name} reputation to ${playerName}`, () => playerAdmin_withSummaryRefresh(() => playersApi.addFactionReputation(dbPlayerId, { factionId: playerAdmin_faction.id, amount: Number(playerAdmin_factionAmount) || 0, confirmation: "ADD FACTION REPUTATION" })), `${playerName}'s faction reputation was updated. Estimated rank may be limited by unfinished faction story progression.`, { actionType: "Give Faction Reputation", target: playerAdmin_faction.name, amount: String(Number(playerAdmin_factionAmount) || 0) }), !dbPlayerId)}
        </div>)}
      ...
      {playerAdmin_activeTab === "Admin" && <div className="playerAdmin_content"><section className="playerAdmin_box"><h4>Player Admin Actions</h4><p>Use this area for player maintenance and high-impact admin actions. Some actions require the player to be online, while database repairs require the player to be offline.</p><PlayerFactionAssignment playerId={dbPlayerId} playerName={playerName} currentFaction={String(playerAdmin_profile.faction || "Neutral")} guild={playerAdmin_profile.guild} supported={playerAdmin_capabilities.assignFaction === true} confirmAction={confirmAction} onRefresh={() => { onRefresh(); playerAdmin_setSummaryRefreshKey((current) => current + 1); }} onActionLog={(actionType, target, amount, notes) => playerAdmin_addLog(actionType, target, amount, notes)} /><div className="playerAdmin_section playerAdmin_repairSection"><h5>Repair</h5><div className="playerAdmin_repairRow"><span className="playerAdmin_repairLabel"><span>Repair Gear</span><em>{playerAdmin_isOnline ? "The player must be offline." : "Equipped and carried gear durability. Relog required."}</em></span><button disabled={!dbPlayerId || playerAdmin_isOnline || playerAdmin_actionResult?.pending} onClick={async () => {
