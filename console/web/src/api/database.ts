...
export type DatabaseRoutineDefinition = Pick<DatabaseRoutine, "oid" | "schema" | "name" | "kind" | "arguments"> & { definition: string };
...
export const databaseApi = {
  ...
  routines: (schema = "dune", q = "") => api<DatabaseRoutine[]>(`/api/database/routines?schema=${encodeURIComponent(schema)}&q=${encodeURIComponent(q)}`),
  routineDefinition: (oid: string) => api<DatabaseRoutineDefinition>(`/api/database/routines/${encodeURIComponent(oid)}`),
