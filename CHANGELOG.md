...
- **`ServerLoginPassword`/`ServerPassword` no longer passed as plaintext `docker run` arguments** (GHSA-fc89-h24v-6j3x, #252). These two values were previously visible in plaintext to anyone with `ps aux`/`/proc/<pid>/cmdline` access or `docker inspect --format '{{json .Args}}'` access — now delivered via `-e` environment variables instead (bare + `BackendLoginConfiguration__`-prefixed forms), across all three launcher scripts (`spawn-server.sh`, `start-server-survival-1.sh`, `start-server-overmap.sh`), via a new shared helper (`runtime/scripts/sietch-login-password-args.sh`). Confirmed working via real player-join tests against a live deployment. No operator action required — this is a transparent change to how an existing value is delivered to the container, not a new required config key or changed default. **Does not** close the related `docker inspect --format '{{.Config.Env}}'` exposure path (env vars are visible there regardless) — full closure of that path requires the separate Docker-socket-scoping fix tracked in #121. The related `DatabasePassword` exposure (#251) remains unresolved and is tracked separately — two independent env-var key names were tested against a live deployment and both failed; direct Funcom outreach is the only remaining avenue for that value.
