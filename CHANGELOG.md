...
- **New self-scoped `players/playtime` route: total tracked playtime + last-seen timestamp for the caller's own linked character** (companion to `Project-Arrakis/mentat`#364's `/profile` command, "Chronicles of Kanly" rollout, `meta`#64). Self-scoped like `players/me`/`players/faction` (same `inventory:read` capability, moderator tier and up), never takes a target actorId. New `playerPlaytimeSummary()` (`duneDb.js`) mirrors `listPlayers()`'s own live-elapsed-time computation (completed `console_player_playtime.total_seconds`, plus time elapsed since `session_started_at` if the player is still online) so the admin player list and this self-service route never silently diverge on what "total playtime" means. Last-seen uses the same dynamic, schema-tolerant column detection (`playerLastSeenSelect()`) already used elsewhere in this file, rather than hardcoding a column that may not exist on every deployment.
