...
An explicit Layer-1 exit gate remains in the doc's own header: no Layer 2 until a full round finds zero new CRITICAL/HIGH against the immediately-prior round's corrections — **this has not yet happened, twelve consecutive passes in**. Whether to run round 9, treat a scoped/mechanical approach as sufficient, or consider this design stable enough to proceed despite the unclosed gate is now an explicit decision for the user, not an automatic continuation.
