...
- **`docs/rw-architecture.md` Layer 1 design revised, then corrected across seven further Eight-Hat audit rounds, a mechanical cross-reference pass, a scoped isolated review, a quick follow-up check, and a coverage-closure pass** (#728-737, #740, then #742-747, then #749-755, then #756-762, then #763-776, then #777, then #778-791, then #792-799, then #803-804, then #806-807, then #808-819, then #820-830, all tracked on #215). The write adapter bridge (#215) and item-catalog endpoint (#222) got a real technical design for the first time. Round 2 found fundamental, independently-cross-corroborated flaws caught before any code was written: an internal credential with no verified binding to loopback traffic and no scoping to the bridge's own routes (4 of 8 hats), a tier ladder the existing IAM machinery cannot actually enforce (2 of 8 hats), and two real functional bugs already in the route table (`ban`/`unban` inverted; `give-item` targeting the wrong Core endpoint — 2 of 8 hats each). Round 3, re-auditing the round-2 corrections, found the credential fix itself was broken in two opposite directions under real deployment configs (fixed by moving the loopback channel to a Unix domain socket instead of TCP), a new routing bug in `guild.remove`, a bot-side-only freshness promise with no Core enforcement, and an idempotency-cache concurrency race the persistence fix hadn't closed. Round 4, auditing the Unix-socket redesign itself with fresh-eyes rigor, found more CRITICAL bugs than round 3 did: a `roleSnapshotAt` fix that silently widened a *shared* signed-fields array consumed by every actor-signed Discord route (a mistake this codebase already had an explicit precedent, #691, warning against), a pre-existing `ADMIN_ALLOWED_IPS` gate that would have silently 403'd all write-bridge traffic for a real documented config, and this project's own default root-UID container config collapsing the socket's trust boundary — plus a global (not per-key) idempotency-queue DoS vector and a stale-socket-file crash risk. Round 5, auditing round 4's own fixes, again found real gaps in the immediately-prior remediation: an `opts` parameter that could reach `undefined` and hang (not crash) every console request once `ADMIN_ALLOWED_IPS` is set (empirically reproduced, CRITICAL), the `ADMIN_ALLOWED_IPS` write-bridge exemption scoped too broadly and unimplementable as described without explicit function-signature threading (HIGH), the new per-key idempotency `Map` never removing settled entries — an unbounded-memory-growth bug independently caught by four separate hats (HIGH), a `secondFactorStore.js` corruption-handling citation that was backwards (it fails closed, not open — HIGH), one of two `clear-backpack` wording fixes from round 4 left unapplied (HIGH), and incomplete operator-facing coverage of the root-UID boot refusal (HIGH) — plus a TOCTOU chmod-timing window and a stale-socket split-brain risk (MEDIUM). Round 6, auditing round 5's own fixes, found the largest set of any round yet — 1 CRITICAL + 12 HIGH: round 5's `ADMIN_ALLOWED_IPS` fix threaded an `opts` flag into `handleApi`/`auth.requireAuth`, but the real `config.allowedIps` gate runs earlier, in `server.js`'s outer request handler, before either function is ever called — CRITICAL #750 was still effectively unfixed as specified (independently found by two hats); this design doc's own header and issue-tracking table had gone stale, never updated to reflect round 5 (the exact failure the doc's own exit gate exists to prevent); a stale `history-clear` confirmation-phrase citation survived a third location, three rounds after being fixed elsewhere; the `warn` command's confirmation requirement contradicted itself between two sections, and a tier table listed two capabilities that don't exist anywhere else in the design; a canonical code example was never updated for round 5's own signed-fields fix, leaving the one call site that actually verifies write-bridge signatures uncovered; the idempotency-cache cleanup fix could delete a still-in-flight mutation's coordination entry on a client timeout, and the cache's periodic pruning sweep had no described synchronization with the code that writes to it — both real double-execution risks; a wrong IAM-action citation caused a miscount in round 5's own new cross-IAM-consistency finding; and the Unix-socket liveness probe had no timeout handler, risking a silent hang of Core's entire boot, not just the write bridge. A mechanical cross-reference script (`docs/design/scripts/rw-architecture-consistency-check.py`, issue #777) was then written after the user questioned why several round-6 findings had survived unchanged since the doc's original commit — investigation confirmed a real per-pass hat-review coverage gap for cross-reference bugs, not something round count alone fixes. The script found two more real gaps (`fill-water` missing from Section 4's lists, `ban`/`spawn`/`despawn`/`respawn` missing inline confirmation-phrase mentions) in minutes. Round 7, auditing round 6's fixes AND #777's own governance trail, found the highest CRITICAL count of any round yet — 2 CRITICAL + 11 HIGH: round 6's two idempotency-cache fixes (#769/#770) had an unstated ordering dependency where both plausible resolutions were broken, either reopening double-execution or stalling every concurrent mutation indefinitely (fixed by mandating the persisted write be awaited inside the mutation before settlement, and bounding the shared lock with a timeout); the liveness-probe timeout fix (`sock.destroy()` with no argument) never actually fired `'error'`, so the probe's own Promise never settled — reproducing, one level up, the exact whole-console-boot-hang the fix was written to prevent, found independently and empirically by two hats (fixed: `destroy()` must be called with an `Error` argument); the brand-new `stop` dual-confirmation gate designed in round 6 was substantially redesigned after five different hats found it had no fallback for single-admin deployments (plausibly the majority profile for this self-hosted fork), a `userId`-based exclusion one operator with two Discord accounts could defeat, an unrealistic 30-second response window never reconciled with the 60-second nonce TTL, an enforcement mechanism ambiguous against the existing nonce store's single-actor binding, and zero test coverage despite an explicit same-paragraph commitment to add three tests; a new `resolveWriteBridgePrincipal()` function (round 6's own CRITICAL fix) had an unspecified path-computation source and no dedicated unit test; the `runExclusive()` idempotency-cache lock was unbounded, recreating round 4's already-rejected global-queue DoS class; Section 3.8 never resolved a `fields`/`signedFields` naming ambiguity that could reproduce round 6's own #768 signature-verification bug via a different mechanism; and #777's own mechanical-pass remediation was found to not have been governed with the same discipline as a hat round — no comment posted on the tracking issue, no CHANGELOG entry — a recurrence of the exact staleness pattern round 6 was created to prevent, one commit later (now fixed retroactively, and the doc's exit-gate paragraph extended to explicitly require this discipline for every remediation pass, not just numbered hat rounds). All CRITICAL/HIGH findings from all seven passes resolved in the current revision. **Before committing to a full round-8, a deliberately narrower isolated review** (4 hats — Security, Architect, UI/UX, QA — scoped to only the `stop` dual-confirmation gate, the newest and least-battle-tested mechanism in the document) was dispatched, and found round 7's own redesign of that gate structurally broken: no wire-level mechanism correlated the second confirmer's request to a specific pending stop request (CRITICAL); idempotency-key semantics for the two confirmation cycles were unspecified, with a plausible path where a genuinely double-confirmed stop silently never executes (CRITICAL); the single-admin fallback decided whether the safety control applied at all using unsigned, unbounded bot-cached role data — the exact class of bug `roleSnapshotAt` was built to prevent (HIGH); "Core's `stop` handler" was ambiguous enough to risk modifying the real, shared `/api/server/stop` route the web console also calls directly (HIGH); the real, already-shipped bot-side button-click code enforces the opposite authorization rule the second confirmer needs (part of a MEDIUM/LOW batch); and the second confirmer's flow was ambiguous enough to require two clicks instead of one (HIGH). The gate was redesigned a second time to reuse the existing, already-hardened nonce store instead of inventing a new record type and a second full preview/execute round-trip — the nonce itself is now the correlation mechanism, only one call ever reaches the real loopback, and the whole gate is explicitly reframed as additive process friction on top of an already-fully-authorized action (matching this document's existing `confirmPhrase` framing) rather than an independent security boundary, which is what makes the bot-cached single-admin determination safe. All CRITICAL/HIGH from this isolated review resolved in the current revision. **A quick two-hat follow-up check** (Security, Architect), scoped to just the redesigned nonce-reuse mechanism, came back with the smallest finding set of any pass to that point: an atomicity gap in the lookup-verify-consume sequence (MEDIUM #803) and a batch of doc cross-reference/unstated-edge-case gaps (LOW #804), both fixed. **Rather than dispatching a full round 8 next, the user flagged the reactive round-over-round pattern as wasting time and asked for a coverage plan instead** — a bounded, numbered plan was built and approved: extend the mechanical script, run two targeted checks on specific acknowledged gaps, then exactly one final full confirmatory round, then a hard stop. The mechanical script, extended to verify every command's real endpoint (method+path) against `server.js`'s actual dispatch table, immediately found two regressions of already-fixed CRITICAL/HIGH bugs — `ban`'s row had regressed to the exact ban/unban-inverted bug round 2 fixed once, and `guild.remove`'s row had regressed to the exact missing-path-segment bug round 3 fixed once (#806, HIGH) — for the cost of a Python extraction routine, not another hat dispatch. Two long-standing, explicitly-acknowledged gaps that had sat untouched since round 2/3 were then closed with one scoped hat check each (#807, MEDIUM): the item-catalog endpoint's design, previously a bespoke prefix-match inconsistent with this codebase's only real precedent, now reuses the already-shipped `listCatalogItems()`; the `WRITE_ACTION_ROUTES` self-check gained a designed (not yet implemented) `auditAction` field, mechanically verifiable against real handler-identity fingerprints.

**The plan's final step — one full, unscoped Eight-Hat round against the whole further-corrected document — found the largest raw finding count of the entire audit history: 1 CRITICAL + 10 HIGH + 6 MEDIUM + 1 LOW (18 distinct findings), filed as #808-819.** A real cancellation gap in `runExclusive()`: timing out a queued operation doesn't actually stop the in-flight `writeJsonAtomicAsync()` I/O, since that function has no `AbortSignal` (CRITICAL #808, fixed with a monotonic generation counter checked immediately before each operation's final `rename()` step). The `stop` gate's first-confirmation branch was missing an explicit check that the calling actor matches the nonce's own `actorUserId` (#809), and its atomicity claim named an outcome ("must be synchronous/atomic") without stating the actual mechanism — zero `await` statements between lookup and consume, now called out explicitly with a required inline code comment warning against introducing one (#810). The catalog endpoint's refresh-policy paragraph still described a design this doc no longer uses, contradicting its own reused-function fix (#811). This doc's own header, Section 8 tracking table, and this changelog had gone stale again — the third recurrence of the exact governance failure rounds 6 and 7 each found in themselves (#812, fixed in this entry). No UI mechanism had ever been designed for a typed-confirmation command despite Section 4 requiring one — designed now around Discord's native Modal component (#813). The catalog endpoint's match semantics were described as name-only when the real `listCatalogItems()` also matches `id`/`category` and does no result ranking, documented as an accepted Layer-1 limitation rather than adding new ranking logic (#814). The `stop` gate's terminal outcomes (success, cancellation) never updated the primary's own original message, leaving it on a stale "waiting" state (#815). Three test-coverage gaps were closed: no test proved the `auditAction` mechanical check itself (#816), no test proved the `runExclusive()` cleanup-ordering fix specifically rather than just its symptoms (#817), and no test asserted `WRITE_ACTION_MIN_TIER` stays at or above what `policy.js`'s `DEFAULT_POLICIES` would independently require (#818). A batched MEDIUM/LOW set (#819) added a rate limit to the catalog endpoint, resolved a Section 2/4 contradiction over `grant-all`'s confirmation classification (conservatively, by strengthening Section 2 to match Section 4's typed-phrase requirement), bounded the pruning sweep's batch size, added a method axis to `resolveWriteBridgePrincipal`'s test matrix, replaced the generic success embed with command-specific result content for `grant-all`/`despawn`/`stop`, corrected a stale path-computation citation, and added stated preview content to several bare "Yes" confirmation cells. All CRITICAL/HIGH resolved in this revision.

**The user then explicitly asked for one more full round (round 8) rather than stopping at the coverage-closure plan's own hard-stop checkpoint.** Round 8, dispatched as all 8 hats against the whole document, again did not come back clean — 1 CRITICAL + 9 HIGH + 6 MEDIUM/LOW (16 distinct findings), filed as #820-830. A corroborated CRITICAL: DBA and Security hats independently found the final confirmatory round's own #808 fix (a generation-counter check-then-rename approach) still allowed two `rename()` calls to race, since a check performed before an awaited, uncancellable call cannot be atomic with that call's own completion (#820, fixed by keeping the queue genuinely serialized on real completion rather than detecting-and-skipping after the fact — this also resolved two knock-on MEDIUM findings, an orphaned temp file and an unstated pruning-sweep resume-cursor interaction, as side effects). Cloud Security and Network hats independently found the item-catalog endpoint had no actor-authentication mechanism at all for its GET method, also defeating its own new rate limit — fixed by switching to `POST /api/items/search` with signature verification (#821); Security and Network hats independently found the catalog rate limit cited a storage cap, not a real throttle — fixed by reusing `applyMutationRateLimit()` (#822). UI/UX found the Modal design silently dropped `restart`/`stop`'s pre-existing 30s cancellable countdown (#823) and left the `stop` gate's second-confirmer typed-phrase requirement unstated (#824) — both fixed; also found the catalog's "likely small enough" no-ranking justification was factually false against the real 2,558-item catalog — fixed with a truncation-signal field (#825). QA and DBA independently found the `policy.js`/`WRITE_ACTION_MIN_TIER` drift test's correspondence was unformalized — fixed with a new `policyAction` schema field mirroring `auditAction` (#826). QA found the Modal UI, command-specific success embeds, and catalog rate limit had zero Section 6 test coverage — fixed (#827). Architect found the Section 8 idempotency-cache tracking row itself had gone stale, contradicting #812's own same-commit "fixed" claim (#828), and that the Modal design conflicted with Discord's single-response constraint against the real button-confirmation precedent — stated explicit as safe-by-construction via the existing single-use nonce (#829). A batched MEDIUM/LOW set (#830) named an unspecified `despawn` embed field, added a caveat to #811's catalog-image freshness claim, and required the stop-gate regression test to use the doc's own established controlled-synchronization-point technique. All CRITICAL/HIGH/MEDIUM/LOW resolved in this revision.

An explicit Layer-1 exit gate remains in the doc's own header: no Layer 2 until a full round finds zero new CRITICAL/HIGH against the immediately-prior round's corrections — **this has not yet happened, twelve consecutive passes in**.

**Round 9 (2026-08-08/09-11, 15 findings, #918-932) was dispatched but never folded into this document's prose until this entry.** A self-verification pass separately reconciled the doc's `discordActorTier()` lowest-tier naming with the console's already-completed observer→player fold (`policy.js`/`services/discordAdapter.js`), and issue #217 (`DUNE_DISCORD_WRITES_ENABLED` parse mismatch between Core and the bot) was closed with a real code fix (`mentat` PR #394) plus doc updates on both sides — both are genuinely resolved, unrelated to round 9's still-open findings.

**2026-09-21/22: Phase 0/Phase 1 tracker reconciliation.** A 9-round history in which every finding's own prose claimed "Fixed" but the tracking issues themselves were never closed left 114 open GitHub issues against a document that had, in fact, resolved the great majority of them. Phase 0 (a dispatched fork) closed 87 of those 114 with an evidence comment citing the exact current doc text that resolved each one, posted as a disposition table + STRIDE table on #215. Phase 1 (posted directly) made the explicit decision this document's own header now reflects: round 9's remaining 15 findings become Phase 2 implementation acceptance criteria, not another full-round design-prose pass — breaking the twelve-consecutive-rounds-zero-implementation pattern documented above. 22 issues remain open, correctly (per #215's Phase 0 comment): 4 core tracking issues (#215/#216/#217→closed since/#222) with zero implementation, 3 issues the doc's own Section 5 status table already calls genuinely open, and 15 round-9 findings now deferred to Layer 2.

**2026-09-22, two scoped reviews during Phase 2 implementation start (not full Eight-Hat rounds, do not claim to satisfy the exit gate above) — see Section 8's own new entries and #215's comments for full detail.** Implementation work on #215 surfaced a same-session design mistake conflating two separate credential problems (the incoming Discord-bot request to `write/preview`/`write/execute`, vs. Core's own internal loopback call to perform the real mutation) — resolved by an 8-hat credential-mechanism audit (no new credential needed for the incoming request; the existing `requireDiscordBotToken()`+`verifyActorSignature()`+`requireDiscordCapability()` chain, already gating `BROADCAST`/`PLAYERS_LINK`, is correct) and an 8-hat two-hop audit (confirmed Hop A/B are genuinely disentangled, found and fixed a real audit-attribution gap — #1010 — and a previously-unstated cross-hop invariant — #1011). A follow-up question (should this section's Unix-socket design be replaced with a properly-scoped `apiKeys.js` reuse) was adjudicated by 5 hats (Architect, Security Architect, Network, Cloud Security, DBA), unanimously retaining the Unix-socket design: `apiKeys.js` reuse would reintroduce CRITICAL #742's bug in a harder-to-fix form, and has no per-key tier field (every key is hardcoded `tier: "owner"`), which would silently discard the entire per-actor tier ladder as an independent check. #1010 and #1011 are fixed in this revision. A real, broader-than-#1007 CRITICAL gap in `apiKeys.js`'s own crown-jewel deny-list (unrelated to this design's correctness) was found as a side effect and filed separately (#1009); #1007 itself was corrected (its actual scope covers 3 of 6 originally-cited actions, not 6).

Whether to run a full round 9-equivalent Eight-Hat pass, or proceed with Phase 2 implementation checking round 9's findings against real code as it's written (the Phase 1 decision), remains the operative choice — Phase 1 already decided this in favor of the latter.
...
- **New staff-only `players/cheater-tracking` route: anti-cheat flag history for a specific target player** (issue #934, companion to `Project-Arrakis/mentat`#361, "Chronicles of Kanly" trust-role rollout, `meta`#64). Unlike every other `players/*` adapter route, this is deliberately NOT self-scoped -- the request body names an explicit target `actorId` (the applicant under review), never the calling staff member's own linked character, since a staff member vetting a Swordmaster/Sietch Guard application is reviewing someone else. New `playerCheaterTracking()` (`duneDb.js`) resolves `dune.cheater_tracking` (keyed by the target's stable `dune.accounts.user`/"FLS" id, not by actor/controller id -- schema verified directly against a live instance, not assumed) via `resolvePlayerMutationTarget` -> `dune.accounts.user`, failing open (empty result) rather than throwing when the account has no FLS id -- a deliberate difference from `playerTeleportIdentity()`'s stricter, fail-closed resolution (see that function's own comment in `duneDb.js` for why). New `CHEATER_TRACKING_READ` capability, deliberately admin/owner tier only (excluded from moderator's grant set, same reasoning as the existing `OPS_*` capabilities) since this discloses another player's anti-cheat record. Every read is audited (`discord.trust.cheater_tracking_read`).

- **New staff-only `players/item-audit-log` route: item-movement history for a specific target player's inventories** (issue #935, companion to `Project-Arrakis/mentat`#368, "Chronicles of Kanly" trust-role rollout, `meta`#64). Like `players/cheater-tracking` (#934), this is deliberately NOT self-scoped -- the request body names an explicit target `actorId`, since Mentat calls this to proactively cross-reference base theft against new Exchange listings, not for a player to look up their own history. Unlike `cheater-tracking`, this is gated at moderator tier and up (`ITEM_AUDIT_LOG_READ`), the same sensitivity class as the existing `INVENTORY_READ`/`STORAGE_READ`/`GUILD_READ` moderator grants. `dune.item_audit_log` grows unbounded and had no index beyond its own primary key (issue #936) -- every query is time-windowed (default 7 days, capped at 30) and row-capped (default 200, capped at 500); the new `console/api/src/duneDb.js` query function also self-heals two missing indexes (`logged_at`, `inventory_id`) via `create index concurrently if not exists` on first use (once per process, not per request), per the operator's explicit sign-off on #936 (indexes are transparent to the closed-source game server's own reads/writes, unlike the command-auth-token incident's value-desync risk). No down-migration exists for these indexes -- see `duneDb.js`'s own comment for the manual `DROP INDEX CONCURRENTLY` rollback if one is ever needed. The table has no `fls_id`/`actor_id` column of its own -- the join path to a specific player is `inventory_id` -> `dune.inventories.actor_id` (already indexed). Every read is audited (`discord.trust.item_audit_log_read`).

- **The hosted-bot auto-invite flow's Discord Application ID is now env-overridable** (`AUTO_INVITE_DISCORD_CLIENT_ID`, `config.autoInviteDiscordClientId`, issue #903) instead of a bare hardcoded literal in `autoInvite.js`. Sahir Venn's client_id remains the correct default for this fork's own deployments; the override exists so a self-hoster running their own hosted-bot backend can point the flow at their own Discord Application, matching every other hosted-bot config value's own env-override pattern. The OLD/Advanced flow's separate "Add to Discord" button (`DiscordBotSection.tsx`) previously hardcoded the same client_id a second, independent time -- now built from the same backend-reported value (`GET /api/settings/discord-bot`'s new `autoInviteDiscordClientId` field), so overriding the env var can never leave that button silently inviting the wrong bot. `buildAutoInviteAuthorizeUrl()` now takes `clientId` as a caller-supplied parameter rather than reading a module constant.

- **Stage 3 of the age-based secrets library rollout: the hosted-bot wizard's Discord OAuth client secret (`discord-hosted-bot-oauth-client-secret`) can now be migrated to optional at-rest encryption**, alongside Stage 2's existing `server-login-password-secret`/`username-server-login-secret` (issue #901). Deliberately NOT built on Stage 2's `_resolve_stage2_secret` pattern: that helper auto-generates a fresh value when the legacy file is absent, which is correct for an auto-generated login password but would be a real correctness bug for an operator-supplied Discord Application credential (it would report `hostedBotOAuthConfigured: true` for a connection that was never actually set up). New `runtime/scripts/lib/console-secrets-env.sh` (a small, console-specific resolver, deliberately separate from `runtime-env.sh`'s game-server-oriented resolvers) is sourced by `runtime/scripts/console.sh`'s new `prepare_discord_hosted_bot_oauth_secret()`, resolved on the **host** before `docker compose up` starts the console -- the age identity lives outside the repo by design and is never bind-mounted into the console container, so decryption cannot happen container-side the way an earlier draft of this change attempted. `docker-compose.web.yml` gained one new `DISCORD_HOSTED_BOT_OAUTH_CLIENT_SECRET` passthrough entry (Compose's own precedence already prefers a real shell-exported value over `.env`, so this is additive/opt-in only). `secrets-cli.sh`'s allow-list extended from 2 to 3 names. Docs updated: `docs/security/age-secrets.md` (migration walkthrough), `docs/security/secrets-management.md` (this secret was missing from the credential inventory entirely -- added). New `runtime/tests/test-secrets-stage3.sh`, added to `tests/security-pr-checks.sh`'s shellcheck file list alongside `console.sh` and the new library file (neither was previously covered).
...
- **New `guilds/faction-summary` route: a tally of each linked player's real IN-GAME GUILD's faction across many Discord users at once** (issue #699), so Project-Arrakis/mentat can auto-derive its own per-Discord-server cosmetic themed-embed faction from real membership instead of a manual setting. This tallies `dune.guilds.guild_faction` (a small player-run in-game organization, max 32 members, at most one per player) -- a genuinely different game concept from an individual player's own personal `dune.player_faction` (which can have thousands of members); an earlier draft of this route conflated the two before merging and was corrected. The bot supplies the Discord server's real member IDs (Core has no concept of Discord servers at all); a new `getGuildFactionTally()` (`duneDb.js`, one batched query resolving all IDs to characters via the same single-link-then-multi-account-default precedence as `getLinkedPlayer()`, then one query joining `dune.guild_members`/`dune.guilds` with the same defensive column resolution `leadershipGuildFactions()` already uses, excluding Neutral guilds) and `guildFactionSummaryProvider()` (`linkProvider.js`) return aggregate counts only (`{ tally: { "<faction name>": count }, consideredCount }`) -- never a per-user mapping or an in-game guild name. This is a genuinely new category of aggregate disclosure (no other Discord-facing route exposes anything about in-game guild membership today), gated at the existing `GUILD_READ` capability (same tier as `guild-storage`/`guild-find`) accordingly. Deliberately does not consult per-guild `discord_account_link_guild_state` enable/default overrides, matching every other player-facing route today (none of them do either — a separate, pre-existing gap, not conflated with this feature).

- **Two new Discord adapter route families wired up: `players/faction` (read-only, auto-detected) and `guild-character-grants/{enable,disable,default}`** (issue #696). Both were declared on Project-Arrakis/mentat's client side (`config.js` `DEFAULT_PATHS`, real bot commands `/dune player faction`, `/dune player enable|disable|default`) with no backing route on this fork at all, so those four bot commands 404'd unconditionally.
  - `players/faction`: `playerFactionProvider()` (`linkProvider.js`) reports the caller's real, current faction from `dune.player_faction`/`dune.factions` (new `getPlayerRealFaction()` in `duneDb.js`, same query pattern as `leadershipCurrentFactions()`). Deliberately never accepts or stores a caller-supplied faction value — it reflects real game state, not a user preference, per an explicit design decision that the bot's own live command's stale description ("Set your faction for themed embeds") does not match; `commandCatalog.js`'s entry documents the real, corrected behavior. Gated by `INVENTORY_READ` (same tier floor as `/dune player whoami`).
  - `guild-character-grants/*`: per-(Discord guild, linked character) enable/disable/default state, in a new `console.discord_account_link_guild_state` table (migration in `migrateDiscordAdapterSchema()`), distinct from the existing global-across-all-guilds `discord_account_links.is_default`. A character absent from this table is implicitly enabled (a missing row and `enabled = true` read identically). Disabling a character that is currently a guild's default clears the default too; setting a character as a guild's default force-enables it. New `setGuildCharacterEnabled()`/`setDefaultLinkedAccountForGuild()` (`duneDb.js`) and `guildGrants{Enable,Disable,Default}Provider()` (`multiAccountLinkProvider.js`). `guildId` is taken from the signed actor object (`actor.guildId`), never a request body field, so a caller cannot claim to act on behalf of a guild it isn't in. Self-scoped (`ACCOUNT_LINK_WRITE`, `requireActorSignature: true`), same gate as `players/accounts/unlink`. `PLAYERS_ACCOUNTS_SET_DEFAULT`'s catalog entry (a separate, uncalled global-default route that happens to also want subcommand name "default") was renamed to `set-default-account` to avoid colliding with `GUILD_GRANTS_DEFAULT`, which is now the one real bot command actually using that subcommand name.
  - New route constants + `DISCORD_LIVE_ADAPTER_ROUTES` entries (`adapter.js`), dispatch entries (`routes.js`), `commandCatalog.js` metadata (required — `buildCommandCatalog()` hard-errors on a live route with no metadata entry). New unit tests (`discordLinkProvider.test.js`, `discordMultiAccountLinkProvider.test.js`) and end-to-end HTTP-path regression tests (`discordAdapter.test.js`) covering RBAC enforcement, the guild-scoping invariant, and the disable-clears-default/default-force-enables behaviors. A narrow, documented naming-lint exception was added for `guild-character-grants` (the URL contains "grants", flagged by an existing route-name safety check) since the path is a fixed external wire contract the bot already ships against. Full backend suite (1947 tests) re-verified green.

### Fixed

- **Live game DB schema drift from the latest Funcom Steam release fixed** (issue #963). A full before/after schema diff (a scratch-restored pre-release backup vs. the live database) found three real breakages, all fixed here in line with upstream's own `v1.4.31` fix for the same release:
  - `dune.markers`'s old composite `marker` column (`(m.marker).x`/`.marker_type`/etc.) is gone, replaced by top-level `marker_type` and a `position` composite (named `vector`, but a plain 3-field double-precision type -- not the pgvector extension). `liveMapPoiMarkers()` and the vehicle-region nearest-marker lookup (`duneDb.js`) hard-failed on every call -- every "World" POI category on the Live Map (Ores, Scrap, Plants, POIs, House Rep, Trainer, Fortress, Hazard Zones, Enemy Camp) and ops-addon vehicle-region assignment were broken. Fixed at both call sites; `liveMapPoiMarkers.integration.test.js`'s real-Postgres fixture updated to match the new schema.
  - `dune.resourcefield_state` lost its `field_kind_id` discriminator column (Spice Blows vs. Flour Sand), silently emptying the Live Map's spice layers (already `.catch()`-guarded, so no visible error). Recovered via `value_remaining`: flour sand never leaves its one fixed tier (60,000), so "not exactly 60,000" is the correct spice complement once the column is gone. `liveMapSpiceFieldRows()`/`liveMapFlourSandFieldRows()` and the addon's `resourcesSectionForDisplayMap()` now probe for the column via `columnsFor()` and fall back automatically, so this also still works unmodified against an older, not-yet-updated schema.
  - This fork's `runtime/sql/patch-coriolis-base-backup-preservation.sql` Coriolis compatibility patch (works around Funcom's own Deep Desert base-backup deletion bug) stopped recognizing Funcom's function shape after `actor_state` was folded into a plain `actors.state` column -- its designed-safe fail-closed exception would have made the next database-update flow report failure on its last step even though the real migration succeeded. Funcom's own rewritten function already natively excludes `BaseBackup` actors, so player data was never actually at risk; the patch's shape-detection regex now recognizes both the old join-based (`s.state IS DISTINCT FROM 'X'`) and new column-based (`a.state <> 'X'`) forms uniformly.

  `dune.spicefield_types` (also dropped) needed no fix -- every read/write path already degrades safely via `tableExists()`. Four other dropped tables (`spicefield_server_availability`, `communinet_player`, `communinet_player_channels`, `removed_items`, `removed_recipes`) are unreferenced anywhere in this fork. New/updated real-Postgres integration test coverage in `console/api/test/coriolisBaseBackupPatch.test.js`, `liveMapPoiMarkers.integration.test.js`, and `liveMapSpiceFieldRows.integration.test.js`.
