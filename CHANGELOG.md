...
- **Landsraad Special Vendor Override (fork-only, issue #907).** A new "Special Vendor Override" section in the Landsraad admin panel lets an operator force one of the game's Special Vendor decrees (Vehicles/Weapons/Armor/Utilities) active for the current term, so that vendor's stock is available to players even if no house has organically won the Landsraad cycle -- motivated by low-population self-hosted servers that may never reach the participation threshold to win a term naturally (confirmed via `dune-dev`: 5+ terms with zero organic wins). Resolves the target decree by `decree_name` at apply time rather than a hardcoded id (an install's decree catalog is game content, not guaranteed to assign the same ids elsewhere). Gated by a new, dedicated `landsraad:vendor-override:write` action -- deliberately **not** folded into the existing `landsraad:write` action and Deny-listed at the `admin` tier (owner-only), since this write fabricates a term's win/decree outcome rather than tuning a numeric value within already-live rules (same remediation pattern as `server:restart`/`carepackage:grant-all`, #859). Never overwrites an already-organically-resolved term automatically -- only an explicit, confirmed "Force Now" may, behind its own stronger warning; "Revert" undoes it within the same term. A background reconciler (mirroring the existing Landsraad Milestone Preset feature) can apply this automatically at each new term, and "Rotate" mode cycles through multiple selected vendor types one per term (only one specialized vendor decree can be active at a time -- the real schema has no way to have all four simultaneously). See `docs/design/landsraad-vendor-override-l1-design-2026-09-11.md` for the full design and its Layer 1 Eight-Hats audits (§7: v1, 1 CRITICAL + 10 HIGH; §9: v2 below, 1 CRITICAL + several HIGH; all resolved). Docs updated: `docs/console/API-REFERENCE.md`, `docs/operator-guide.md`. **Not proposed for the Red-Blink upstream** -- this is an operator-choice "cheat" feature, deliberately fork-only.

  **v2, same issue, same PR: optionally also forces `winning_faction_id`/`reigning_faction_id`.** The decree-only write above was confirmed (directly by the operator, matching this fork's own game-mechanic research) to be a complete no-op for every player -- vendor access is gated on which house is *recorded as winning* the term, not just which decree is active. A new optional "Target House" dropdown (Atreides/Harkonnen, resolved by name against a hardcoded allow-list, never a bare `dune.factions` lookup -- `None`/`Smuggler` are real rows in that table but not real Landsraad-eligible houses) also forces the term's winning/reigning house in the same write, reusing the same owner-only action and the same never-overwrite-an-organic-result guard. **Two risks named explicitly, neither fixable from this codebase, both surfaced in the confirm dialog and `docs/operator-guide.md`:** (1) may grant the target house's players real Landsraad rewards, not just vendor access (unconfirmed hypothesis, pending empirical verification); (2) **confirmed** -- a real Postgres trigger (`landsraad_tasks_check_term_won_state` on `landsraad_tasks`) normally promotes a house to "winning" on a real task-board completion, but only while the term's `winning_faction_id` is still `NULL` -- forcing it silently disables that trigger's effect for the rest of the term, masking any house's real win with no error and no way for Revert to recover it. Byte-identical to the decree-only write when no house is selected (verified by a dedicated SQL-capture test, not just behavioral testing). `console.landsraad_vendor_override_state` gained an additive `last_applied_faction_id` column so Revert can tell "this feature set the house" apart from "a real win" the same way it already does for the decree.

  **Confirmed live, 2026-09-11, on `dune-dev`:** the game mechanic is per-house on `winning_faction_id`, and the vendor NPC is always visible -- what's actually gated is whether it will *sell* to a player. Forcing the Vehicle Vendor with Atreides as the target house: Atreides could buy, the other house (no favor) could not. This confirms the v1+v2 combined write produces the intended real effect, not just a successful database write. UI copy in `LandsraadPanel.tsx` corrected from "vendor becomes available/visible" to "vendor will sell" throughout, matching this confirmed mechanic. **Still open:** the separate economy-reward hypothesis (risk (1) above) was not part of this test and remains unconfirmed either way.
