...
- **New `guilds/faction-summary` route: a real-faction tally across many Discord users at once** (issue #699), so Project-Arrakis/mentat can auto-derive its own per-guild cosmetic themed-embed faction from real membership instead of a manual setting. The bot supplies the guild's real Discord member IDs (Core has no concept of Discord guild membership at all); a new `getGuildFactionTally()` (`duneDb.js`, one batched query resolving all IDs to characters via the same single-link-then-multi-account-default precedence as `getLinkedPlayer()`, then one query tallying real `dune.player_faction` values) and `guildFactionSummaryProvider()` (`linkProvider.js`) return aggregate counts only (`{ tally: { "<faction name>": count }, consideredCount }`) — never a per-user mapping, so this cannot be used to learn any individual member's faction beyond what their own `/dune player faction` already discloses. Gated at the existing `GUILD_READ` capability (same tier as `guild-storage`/`guild-find`). Deliberately does not consult per-guild `discord_account_link_guild_state` enable/default overrides, matching every other player-facing route today (none of them do either — a separate, pre-existing gap, not conflated with this feature).
